// Yacht rental Customizer script
...
jQuery(document).ready(function(){
	...
	jQuery('.to_demo_wrap').each(function(idx) {
		"use strict";
		var tab = jQuery(this).parents('.yacht_rental_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
		var info = jQuery(this).parents('.yacht_rental_options_field_info').attr('id', 'to_demo_scheme_'+scheme).attr('data-scheme', scheme).data('scheme', scheme);
		if (idx==0 && jQuery(this).find('.to_demo_pin').length==1 && yacht_rental_get_cookie('to_demo_fixed')==1) yacht_rental_options_fix_demo(info);
	});

	// Stick demo panel
	jQuery(document).on('click', '.yacht_rental_options_field_info .to_demo_pin', function(e) {
		yacht_rental_options_fix_demo(jQuery(this).parents('.yacht_rental_options_field_info'));
		e.preventDefault();
		return false;
	});	

	// Update demo on page load
	yacht_rental_options_decorate_demo();

	// Update demo on change fields
	jQuery('#partition_schemes_content').on('change', '.yacht_rental_options_input,.yacht_rental_options_field input[type="hidden"]', function(e) {
		"use strict";
		var tab = jQuery(this).parents('.yacht_rental_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
		yacht_rental_options_decorate_demo(scheme);
	});

	// Update demo on blur color fields
	jQuery('#partition_schemes_content').on('blur', '.yacht_rental_options_input_color', function(e) {
		"use strict";
		var tab = jQuery(this).parents('.yacht_rental_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
	...
	jQuery('#partition_schemes_content .yacht_rental_options_tab > ul').sortable({
		update: function(event, ui) {
			var tabs = "";
			ui.item.parents('.yacht_rental_options_tab').find('> ul > li').each(function() {
				tabs += (tabs ? YACHT_RENTAL_STORAGE['to_delimiter'] : '') + jQuery(this).attr('id').substr(4);
			});
			ui.item.parents('.yacht_rental_options_tab').find('> .yacht_rental_options_schemes_order').val(tabs);
		}
	}).disableSelection().after('<input type="hidden" name="yacht_rental_options_schemes_order" class="yacht_rental_options_schemes_order" value="">');
});


// When switch tabs - update fixed demo block
function yacht_rental_init_hidden_elements(panel) {
	"use strict";
	var demo = panel.find('.yacht_rental_options_field_info[data-scheme]');
	if (demo.length == 0) return;
	var info2 = jQuery('.yacht_rental_options').find('>.yacht_rental_options_field_info');
...
function yacht_rental_options_action_scheme_delete(obj) {
	"use strict";
	var button = jQuery(obj);
	var tab_id = button.parents('.yacht_rental_options_tab_content').attr('id');
	var scheme = tab_id.substring(4, tab_id.length-8);
	yacht_rental_message_confirm(YACHT_RENTAL_STORAGE['to_strings']['scheme_delete_confirm'], YACHT_RENTAL_STORAGE['to_strings']['scheme_delete']+': "'+scheme+'"', function(btn) {
		"use strict";
		if (btn != 1) return;
		var data = {
			action: 'yacht_rental_options_scheme_delete',
			nonce: YACHT_RENTAL_STORAGE['ajax_nonce'],
			scheme: scheme,
			order: button.parents('.yacht_rental_options_tab').find('> .yacht_rental_options_schemes_order').val()
		};
		setTimeout(function(){
			yacht_rental_message_info(YACHT_RENTAL_STORAGE['to_strings']['recompile_styles'], YACHT_RENTAL_STORAGE['to_strings']['wait'], 'spin3 animate-spin', 60000);
		}, 600);
		jQuery.post(YACHT_RENTAL_STORAGE['ajax_url'], data, function(response) {
			...
			if (rez.error === '') {
				yacht_rental_message_success(YACHT_RENTAL_STORAGE['to_strings']['scheme_delete_complete']+'<br>'+YACHT_RENTAL_STORAGE['to_strings']['reload_page'], YACHT_RENTAL_STORAGE['to_strings']['scheme_delete']);
				setTimeout(function() { location.reload(); }, 3000);
			} else {
				yacht_rental_message_warning(YACHT_RENTAL_STORAGE['to_strings']['scheme_delete_failed']+':<br>'+rez.error, YACHT_RENTAL_STORAGE['to_strings']['scheme_delete']);
			}
		});
...
function yacht_rental_options_action_scheme_copy(obj) {
	"use strict";
	var button = jQuery(obj);
	var tab_id = button.parents('.yacht_rental_options_tab_content').attr('id');
	var scheme = tab_id.substring(4, tab_id.length-8);
	yacht_rental_message_confirm(YACHT_RENTAL_STORAGE['to_strings']['scheme_copy_confirm'], YACHT_RENTAL_STORAGE['to_strings']['scheme_copy']+': "'+scheme+'"', function(btn) {
		"use strict";
		if (btn != 1) return;
		var data = {
			action: 'yacht_rental_options_scheme_copy',
			nonce: YACHT_RENTAL_STORAGE['ajax_nonce'],
			scheme: scheme,
			order: button.parents('.yacht_rental_options_tab').find('> .yacht_rental_options_schemes_order').val()
		};
		...
		jQuery.post(YACHT_RENTAL_STORAGE['ajax_url'], data, function(response) {
			...
			if (rez.error === '') {
				yacht_rental_message_success(YACHT_RENTAL_STORAGE['to_strings']['scheme_copy_complete']+'<br>'+YACHT_RENTAL_STORAGE['to_strings']['reload_page'], YACHT_RENTAL_STORAGE['to_strings']['scheme_copy']);
				setTimeout(function() { location.reload(); }, 3000);
			} else {
				yacht_rental_message_warning(YACHT_RENTAL_STORAGE['to_strings']['scheme_copy_failed']+':<br>'+rez.error, YACHT_RENTAL_STORAGE['to_strings']['scheme_copy']);
			}
		});
...
function yacht_rental_options_fix_demo(info) {
	"use strict";
	var info2 = info.parents('.yacht_rental_options').toggleClass('to_demo_fixed').find('>.yacht_rental_options_field_info');
	if (info2.length == 0) {
		var clonedObj = info.clone();
		clonedObj.removeAttr('id');
		info.parents('.yacht_rental_options').append(clonedObj);
		info2 = info.parents('.yacht_rental_options').find('>.yacht_rental_options_field_info');
	}
	yacht_rental_set_cookie('to_demo_fixed', jQuery('.yacht_rental_options').hasClass('to_demo_fixed') ? 1 : 0, 365);
}


// Decorate demo block
function yacht_rental_options_decorate_demo() {
	"use strict";
	...
	jQuery('.to_demo_body').each(function() {
		...
		var scheme = jQuery(this).parents('.yacht_rental_options_field_info').data('scheme');
