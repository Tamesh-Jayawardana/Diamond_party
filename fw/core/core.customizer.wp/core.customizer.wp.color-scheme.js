...
( function( api ) {
	...
	if (!yacht_rental_customizer_vars['need_refresh']) {
		for (var i in yacht_rental_color_schemes) {
			cssTemplate[i] = wp.template( 'yacht_rental-color-scheme-'+i );
		}
		cssTemplate['fonts'] = wp.template( 'yacht_rental-fonts' );
	}
	...
	api.bind('ready', function() {
		...
		// Add 'Refresh' button
		if (yacht_rental_customizer_vars['need_refresh']) {
			jQuery('#customize-header-actions .spinner').after('<input type="button" class="button customize-action-refresh" value="'+yacht_rental_customizer_vars['msg_refresh']+'">');
		...
		jQuery('#customize-theme-controls .control-section').each(function () {
			yacht_rental_customizer_wp_check_dependencies(jQuery(this));
		});

		// Update colors
		yacht_rental_customizer_wp_change_color_scheme(api('color_scheme')());
	});
	
	// On change any control - check for dependencies
	api.bind('change', function(obj) {
		if (obj.id == 'scheme_storage') return;
		yacht_rental_customizer_wp_check_dependencies(jQuery('#customize-theme-controls #customize-control-'+obj.id).parents('.control-section'));
		yacht_rental_customizer_wp_refresh_preview(obj);
	});

	// Check for dependencies
	function yacht_rental_customizer_wp_check_dependencies(cont) {
		"use strict";
		...
		cont.find('.customize-control').each(function() {
			...
			var depend = false;
			for (var fld in yacht_rental_dependencies) {
				if (fld == id) {
			...
			if (depend) {
				...
				if ((dep_cnt > 0 && dep_cmp == 'or') || (dep_cnt == dep_all && dep_cmp == 'and')) {
					jQuery(this).show().removeClass('yacht_rental_options_no_use');
				} else {
					jQuery(this).hide().addClass('yacht_rental_options_no_use');
				}
			}
		});
	}
	...
	function yacht_rental_customizer_wp_refresh_preview(obj) {
		"use strict";
		...
		if (id == 'color_scheme') {
			yacht_rental_customizer_wp_change_color_scheme(val);
		} else if (updateCSS) {
			var simple = api('color_settings')()=='simple';
			for (var opt in yacht_rental_color_schemes['original']) {
				if (opt == id) {
					updateCSS = false;
					// Store new value in the color table
					yacht_rental_customizer_wp_update_color_scheme(opt, val);
					...
					if (simple) {
						if (id == 'text_link') {
							...
							yacht_rental_customizer_wp_update_color_scheme('alter_link', val);
						} else if (id == 'text_hover') {
							api('alter_hover').set( val );
							api.control( 'alter_hover' ).container.find( '.color-picker-hex' )
								.data( 'data-default-color', val )
								.wpColorPicker( 'defaultColor', val );
							yacht_rental_customizer_wp_update_color_scheme('alter_hover', val);
						}
					}
					updateCSS = true;
					processed = true;
			...
			if (!processed) {
				for (var tag in yacht_rental_fonts) {
					for (var prop in yacht_rental_fonts[tag]) {
						if (prop=='title' || prop=='description') continue;
						if (tag+'-'+prop == id) {
							processed = true;
							// Store new value in the fonts table
							yacht_rental_customizer_wp_update_fonts(tag, prop, val);
							break;
						}
					}
					if (processed) break;
			...
			// Refresh CSS
			if (processed) yacht_rental_customizer_wp_update_css();
		}
	...
	// Store new value in the color table
	function yacht_rental_customizer_wp_update_color_scheme(opt, value) {
		"use strict";
		yacht_rental_color_schemes[api('color_scheme')()][opt] = value;
		api('scheme_storage').set(yacht_rental_serialize(yacht_rental_color_schemes))
	}
	

	// Change color scheme - update colors and generate css
	function yacht_rental_customizer_wp_change_color_scheme(value) {
		"use strict";
		updateCSS = false;
		for (var opt in yacht_rental_color_schemes[value]) {
			if (api(opt) == undefined) continue;
			api( opt ).set( yacht_rental_color_schemes[value][opt] );
			api.control( opt ).container.find( '.color-picker-hex' )
				.data( 'data-default-color', yacht_rental_color_schemes[value][opt] )
				.wpColorPicker( 'defaultColor', yacht_rental_color_schemes[value][opt] );
		}
		updateCSS = true;
		yacht_rental_customizer_wp_update_css();
	}

	// Store new value in the fonts table
	function yacht_rental_customizer_wp_update_fonts(tag, prop, value) {
		"use strict";
		yacht_rental_fonts[tag][prop] = value;
	}
	
	// Generate the CSS for the current Color Scheme and send it to the preview window
	function yacht_rental_customizer_wp_update_css() {
		"use strict";

		if (!updateCSS || yacht_rental_customizer_vars['need_refresh']) return;

		var css = '';
		
		// Add color styles
		for (var scheme in yacht_rental_color_schemes) {
			
			var colors = [];
			
			// Copy all colors!
			for (var i in yacht_rental_color_schemes[scheme]) {
				if (i=='title') continue;
				colors[i] = yacht_rental_color_schemes[scheme][i];
			}
		...
		// Make theme specific colors and tints
		if (window.yacht_rental_customizer_wp_add_theme_fonts) fonts = yacht_rental_customizer_wp_add_theme_fonts(fonts);
