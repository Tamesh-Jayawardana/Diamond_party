<?php
/*
 * Support for the Boats
 */



// Register custom post type
if (!function_exists('trx_utils_support_boats_post_type')) {
	add_action( 'trx_utils_custom_post_type', 'trx_utils_support_boats_post_type', 10, 2 );
	function trx_utils_support_boats_post_type($name, $args=false) {
		if ($name=='boats') {
			if ($args===false) {
				$args = array(
					'label'               => esc_html__( 'Boats', 'trx_utils' ),
					'description'         => esc_html__( 'Boats Description', 'trx_utils' ),
					'labels'              => array(
						'name'                => esc_html__( 'Boats', 'trx_utils' ),
						'singular_name'       => esc_html__( 'Boat', 'trx_utils' ),
						'menu_name'           => esc_html__( 'Boats', 'trx_utils' ),
						'parent_item_colon'   => esc_html__( 'Parent Item:', 'trx_utils' ),
						'all_items'           => esc_html__( 'All Boats', 'trx_utils' ),
						'view_item'           => esc_html__( 'View Item', 'trx_utils' ),
						'add_new_item'        => esc_html__( 'Add New Boat', 'trx_utils' ),
...
if (!function_exists('trx_utils_support_boats_taxonomy')) {
	add_action( 'trx_utils_custom_taxonomy', 'trx_utils_support_boats_taxonomy', 10, 2 );
	function trx_utils_support_boats_taxonomy($name, $args=false) {
		if ($name=='boats_group') {
			if ($args===false) {
				$args = array(
					'post_type' 		=> 'boats',
					'hierarchical'      => true,
					'labels'            => array(
						'name'              => esc_html__( 'Boats Group', 'trx_utils' ),
						...
						'menu_name'         => esc_html__( 'Boats Group', 'trx_utils' ),
					),
					'show_ui'           => true,
					'show_admin_column' => true,
					'query_var'         => true,
					'rewrite'           => array( 'slug' => 'boats_group' )
					);
			}
