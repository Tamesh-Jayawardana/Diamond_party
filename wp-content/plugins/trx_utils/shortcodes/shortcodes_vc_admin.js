jQuery(document).ready(function () {
	...
	if (typeof(vc) != 'undefined' && typeof(vc.shortcode_view) != 'undefined') {
			...
			changeShortcodeParams:function (model) {
				...
				if (_.isObject(params)) {
					if (_.isString(params.count))
						yacht_rental_vc_columns_width(this.$el, params.count);
					else if (_.isString(params.columns))
						yacht_rental_vc_columns_width(this.$el, params.columns);
				}
			},
			changedContent: function(view) {
		...
		window.VcTrxColumnItemView = vc.shortcode_view.extend({
			changeShortcodeParams:function (model) {
				...
				if (_.isObject(params) && _.isString(params.span)) {
					...
					yacht_rental_vc_columns_width(this.$el.parents('.trx_sc_columns'));
				}
			},
			changedContent: function(view) {
				window.VcTrxColumnsView.__super__.changedContent.call(view);
		...
		jQuery('body').on('change', 'select.post_type', function () {
			...
			yacht_rental_admin_fill_categories(fld, cat_fld, cat_lbl);
		});
	}
});
...
function yacht_rental_vc_columns_width(columns) {
