...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('yacht_rental_sc_infobox_theme_setup')) {
	add_action( 'yacht_rental_action_before_init_theme', 'yacht_rental_sc_infobox_theme_setup' );
	function yacht_rental_sc_infobox_theme_setup() {
		add_action('yacht_rental_action_shortcodes_list', 		'yacht_rental_sc_infobox_reg_shortcodes');
		if (function_exists('yacht_rental_exists_visual_composer') && yacht_rental_exists_visual_composer())
			add_action('yacht_rental_action_shortcodes_list_vc','yacht_rental_sc_infobox_reg_shortcodes_vc');
	}
}
...
if (!function_exists('yacht_rental_sc_infobox')) {	
	function yacht_rental_sc_infobox($atts, $content=null){	
		...
		$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') 
				. ' class="sc_infobox sc_infobox_style_' . esc_attr($style) 
					. (yacht_rental_param_is_on($closeable) ? ' sc_infobox_closeable' : '') 
					. (!empty($class) ? ' '.esc_attr($class) : '') 
					. ($icon!='' && !yacht_rental_param_is_inherit($icon) ? ' sc_infobox_iconed '. esc_attr($icon) : '') 
					. '"'
				...
				. trim($content)
				. '</div>';
		return apply_filters('yacht_rental_shortcode_output', $output, 'trx_infobox', $atts, $content);
	}
	add_shortcode('trx_infobox', 'yacht_rental_sc_infobox');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'yacht_rental_sc_infobox_reg_shortcodes' ) ) {
	function yacht_rental_sc_infobox_reg_shortcodes() {
	
		yacht_rental_sc_map("trx_infobox", array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'yacht_rental_sc_infobox_reg_shortcodes_vc' ) ) {
	function yacht_rental_sc_infobox_reg_shortcodes_vc() {
		...
		class WPBakeryShortCode_Trx_Infobox extends Yacht_Rental_VC_ShortCodeContainer {}
	}
}
?>