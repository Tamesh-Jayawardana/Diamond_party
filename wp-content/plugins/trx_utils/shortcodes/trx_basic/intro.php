...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('yacht_rental_sc_intro_theme_setup')) {
	add_action( 'yacht_rental_action_before_init_theme', 'yacht_rental_sc_intro_theme_setup' );
	function yacht_rental_sc_intro_theme_setup() {
		add_action('yacht_rental_action_shortcodes_list', 		'yacht_rental_sc_intro_reg_shortcodes');
		if (function_exists('yacht_rental_exists_visual_composer') && yacht_rental_exists_visual_composer())
			add_action('yacht_rental_action_shortcodes_list_vc','yacht_rental_sc_intro_reg_shortcodes_vc');
	}
}
...
if (!function_exists('yacht_rental_sc_intro')) {	
	function yacht_rental_sc_intro($atts, $content=null){	
		...
		$output = '<div '.(!empty($url) ? 'data-href="'.esc_url($url).'"' : '') 
					...
					. '<div class="sc_intro_inner '.($style ? ' sc_intro_style_' . esc_attr($style) : '').'"'.(!empty($content_width) ? ' style="width:'.esc_attr($content_width).';"' : '').'>'
						...
						. '<div class="sc_intro_content">'
							. (!empty($subtitle) && $style!=4 && $style!=5 ? '<h6 class="sc_intro_subtitle">' . trim(yacht_rental_strmacros($subtitle)) . '</h6>' : '')
							. (!empty($title) ? '<h2 class="sc_intro_title">' . trim(yacht_rental_strmacros($title)) . '</h2>' : '')
							. (!empty($description) && $style!=1 ? '<div class="sc_intro_descr">' . trim(yacht_rental_strmacros($description)) . '</div>' : '')
		...
		return apply_filters('yacht_rental_shortcode_output', $output, 'trx_intro', $atts, $content);
	}
	add_shortcode('trx_intro', 'yacht_rental_sc_intro');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'yacht_rental_sc_intro_reg_shortcodes' ) ) {
	function yacht_rental_sc_intro_reg_shortcodes() {
	
		yacht_rental_sc_map("trx_intro", array(
			...
			"params" => array(
				"style" => array(
					...
					"value" => "1",
					"type" => "checklist",
					"options" => yacht_rental_get_list_styles(1, 5)
				),
				"align" => array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'yacht_rental_sc_intro_reg_shortcodes_vc' ) ) {
	function yacht_rental_sc_intro_reg_shortcodes_vc() {
		...
		vc_map( array(
			...
			"params" => array(
				array(
					...
					"class" => "",
					"admin_label" => true,
					"value" => array_flip((array)yacht_rental_get_list_styles(1, 5)),
					"type" => "dropdown"
				),
				array(
					"param_name" => "align",
		...
		class WPBakeryShortCode_Trx_Intro extends Yacht_Rental_VC_ShortCodeSingle {}
	}
}
?>