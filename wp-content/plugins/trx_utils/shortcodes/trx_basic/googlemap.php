...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('yacht_rental_sc_googlemap_theme_setup')) {
	add_action( 'yacht_rental_action_before_init_theme', 'yacht_rental_sc_googlemap_theme_setup' );
	function yacht_rental_sc_googlemap_theme_setup() {
		add_action('yacht_rental_action_shortcodes_list', 		'yacht_rental_sc_googlemap_reg_shortcodes');
		if (function_exists('yacht_rental_exists_visual_composer') && yacht_rental_exists_visual_composer())
			add_action('yacht_rental_action_shortcodes_list_vc','yacht_rental_sc_googlemap_reg_shortcodes_vc');
	}
}
...
if (!function_exists('yacht_rental_sc_googlemap')) {	
	function yacht_rental_sc_googlemap($atts, $content = null) {
		...
		if (empty($style)) $style = yacht_rental_get_custom_option('googlemap_style');
		$api_key = yacht_rental_get_theme_option('api_google');
        if (yacht_rental_get_theme_option('api_google') != '') {
		    wp_enqueue_script( 'googlemap', yacht_rental_get_protocol().'://maps.google.com/maps/api/js'.($api_key ? '?key='.$api_key : ''), array(), null, true );
		    wp_enqueue_script( 'yacht-rental-googlemap-script', yacht_rental_get_file_url('js/core.googlemap.js'), array(), null, true );
        }
		yacht_rental_storage_set('sc_googlemap_markers', array());
		$content = do_shortcode($content);
		$output = '';
		$markers = yacht_rental_storage_get('sc_googlemap_markers');
		if (count($markers) == 0) {
			$markers[] = array(
				'title' => yacht_rental_get_custom_option('googlemap_title'),
				'description' => yacht_rental_strmacros(yacht_rental_get_custom_option('googlemap_description')),
				'latlng' => yacht_rental_get_custom_option('googlemap_latlng'),
				'address' => yacht_rental_get_custom_option('googlemap_address'),
				'point' => yacht_rental_get_custom_option('googlemap_marker')
			);
		}
		$output .= 
		...
		foreach ($markers as $marker) {
            ...
            if (yacht_rental_get_theme_option('api_google') != '') {
                $output .= '<div id="' . esc_attr($marker['id']) . '" class="sc_googlemap_marker"'
                    ...
                    . ' data-description="' . esc_attr(yacht_rental_strmacros($marker['description'])) . '"'
		...
		return apply_filters('yacht_rental_shortcode_output', $output, 'trx_googlemap', $atts, $content);
	}
	add_shortcode("trx_googlemap", "yacht_rental_sc_googlemap");
}


if (!function_exists('yacht_rental_sc_googlemap_marker')) {	
	function yacht_rental_sc_googlemap_marker($atts, $content = null) {
		if (yacht_rental_in_shortcode_blogger()) return '';
		extract(yacht_rental_html_decode(shortcode_atts(array(
			// Individual params
			"title" => "",
		...
		$content = do_shortcode($content);
		yacht_rental_storage_set_array('sc_googlemap_markers', '', array(
			'id' => $id,
			'title' => $title,
			'description' => !empty($content) ? $content : $address,
			'latlng' => $latlng,
			'address' => $address,
			'point' => $point ? $point : yacht_rental_get_custom_option('googlemap_marker')
			)
		);
		return '';
	}
	add_shortcode("trx_googlemap_marker", "yacht_rental_sc_googlemap_marker");
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'yacht_rental_sc_googlemap_reg_shortcodes' ) ) {
	function yacht_rental_sc_googlemap_reg_shortcodes() {
	
		yacht_rental_sc_map("trx_googlemap", array(
			...
			"params" => array(
				...
				"style" => array(
					...
					"value" => "default",
					"type" => "checklist",
					"options" => yacht_rental_get_sc_param('googlemap_styles')
				),
				"scheme" => array(
				...
				"width" => yacht_rental_shortcodes_width('100%'),
				"height" => yacht_rental_shortcodes_height(240),
...
-------------------------------------------------------------------- */
if ( !function_exists( 'yacht_rental_sc_googlemap_reg_shortcodes_vc' ) ) {
	function yacht_rental_sc_googlemap_reg_shortcodes_vc() {
		...
		vc_map( array(
			...
			"params" => array(
				...
				array(
					...
					"admin_label" => true,
					"class" => "",
					"value" => array_flip((array)yacht_rental_get_sc_param('googlemap_styles')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "scheme",
				...
				yacht_rental_vc_width('100%'),
				yacht_rental_vc_height(240),
		...
		class WPBakeryShortCode_Trx_Googlemap extends Yacht_Rental_VC_ShortCodeCollection {}
		class WPBakeryShortCode_Trx_Googlemap_Marker extends Yacht_Rental_VC_ShortCodeCollection {}
	}
}
?>