...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('yacht_rental_sc_anchor_theme_setup')) {
	add_action( 'yacht_rental_action_before_init_theme', 'yacht_rental_sc_anchor_theme_setup' );
	function yacht_rental_sc_anchor_theme_setup() {
		add_action('yacht_rental_action_shortcodes_list', 		'yacht_rental_sc_anchor_reg_shortcodes');
		if (function_exists('yacht_rental_exists_visual_composer') && yacht_rental_exists_visual_composer())
			add_action('yacht_rental_action_shortcodes_list_vc','yacht_rental_sc_anchor_reg_shortcodes_vc');
	}
}
...
if (!function_exists('yacht_rental_sc_anchor')) {	
	function yacht_rental_sc_anchor($atts, $content = null) {
		...
		$output = $id 
			? '<a id="'.esc_attr($id).'"'
				...
				. ' data-description="' . ($description ? esc_attr(yacht_rental_strmacros($description)) : ''). '"'
				. ' data-icon="' . ($icon ? $icon : '') . '"' 
				. ' data-url="' . ($url ? esc_attr($url) : '') . '"' 
				. ' data-separator="' . (yacht_rental_param_is_on($separator) ? 'yes' : 'no') . '"'
				. '></a>'
			: '';
		return apply_filters('yacht_rental_shortcode_output', $output, 'trx_anchor', $atts, $content);
	}
	add_shortcode("trx_anchor", "yacht_rental_sc_anchor");
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'yacht_rental_sc_anchor_reg_shortcodes' ) ) {
	function yacht_rental_sc_anchor_reg_shortcodes() {
	
		yacht_rental_sc_map("trx_anchor", array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'yacht_rental_sc_anchor_reg_shortcodes_vc' ) ) {
	function yacht_rental_sc_anchor_reg_shortcodes_vc() {
		...
		class WPBakeryShortCode_Trx_Anchor extends Yacht_Rental_VC_ShortCodeSingle {}
	}
}
?>