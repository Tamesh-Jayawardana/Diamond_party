...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('yacht_rental_sc_form_theme_setup')) {
	add_action( 'yacht_rental_action_before_init_theme', 'yacht_rental_sc_form_theme_setup' );
	function yacht_rental_sc_form_theme_setup() {
		add_action('yacht_rental_action_shortcodes_list', 		'yacht_rental_sc_form_reg_shortcodes');
		if (function_exists('yacht_rental_exists_visual_composer') && yacht_rental_exists_visual_composer())
			add_action('yacht_rental_action_shortcodes_list_vc','yacht_rental_sc_form_reg_shortcodes_vc');
	}
}
...
if (!function_exists('yacht_rental_sc_form')) {	
	function yacht_rental_sc_form($atts, $content = null) {
		...
		yacht_rental_enqueue_messages();	// Load core messages
	
		yacht_rental_storage_set('sc_form_data', array(
			'id' => $id,
            'counter' => 0
            )
        );
		...
		$output = '<div ' . ($id ? ' id="'.esc_attr($id).'_wrap"' : '')
			...
			.'<div ' . ($id ? ' id="'.esc_attr($id).'"' : '') 
				. ' class="sc_form'
					...
					. (!empty($align) && !yacht_rental_param_is_off($align) ? ' align'.esc_attr($align) : '') 
				...
				. '>'
					. (!empty($subtitle) 
						? '<h6 class="sc_form_subtitle sc_item_subtitle">' . trim(yacht_rental_strmacros($subtitle)) . '</h6>' 
						: '')
					. (!empty($title) 
						? '<h2 class="sc_form_title sc_item_title' . (empty($description) ? ' sc_item_title_without_descr' : ' sc_item_title_without_descr') . '">' . trim(yacht_rental_strmacros($title)) . '</h2>' 
						: '')
					. (!empty($description) 
						? '<div class="sc_form_descr sc_item_descr">' . trim(yacht_rental_strmacros($description)) . ($style == 1 ? do_shortcode('[trx_socials size="tiny" shape="round"][/trx_socials]') : '') . '</div>' 
						: '');
		
		$output .= yacht_rental_show_post_layout(array(
												'layout' => $style,
												'id' => $id,
												'action' => $action,
												'content' => $content,
		...
		return apply_filters('yacht_rental_shortcode_output', $output, 'trx_form', $atts, $content);
	}
	add_shortcode("trx_form", "yacht_rental_sc_form");
}

if (!function_exists('yacht_rental_sc_form_item')) {	
	function yacht_rental_sc_form_item($atts, $content=null) {
		if (yacht_rental_in_shortcode_blogger()) return '';
		extract(yacht_rental_html_decode(shortcode_atts( array(
			// Individual params
			"type" => "text",
		...
		yacht_rental_storage_inc_array('sc_form_data', 'counter');
	
		$class .= ($class ? ' ' : '') . yacht_rental_get_css_position_as_classes($top, $right, $bottom, $left);
		if (empty($id)) $id = yacht_rental_storage_get_array('sc_form_data', 'id').'_'.yacht_rental_storage_get_array('sc_form_data', 'counter');
		...
		if ($type == 'textarea')
			...
			wp_enqueue_script( 'jquery-picker', yacht_rental_get_file_url('/js/picker/picker.js'), array('jquery'), null, true );
			wp_enqueue_script( 'jquery-picker-date', yacht_rental_get_file_url('/js/picker/picker.date.js'), array('jquery'), null, true );
			...
			wp_enqueue_script( 'jquery-picker', yacht_rental_get_file_url('/js/picker/picker.js'), array('jquery'), null, true );
			wp_enqueue_script( 'jquery-picker-time', yacht_rental_get_file_url('/js/picker/picker.time.js'), array('jquery'), null, true );
		...
		return apply_filters('yacht_rental_shortcode_output', $output, 'trx_form_item', $atts, $content);
	}
	add_shortcode('trx_form_item', 'yacht_rental_sc_form_item');
}

// AJAX Callback: Send contact form data
if ( !function_exists( 'yacht_rental_sc_form_send' ) ) {
	function yacht_rental_sc_form_send() {
	
		if ( !wp_verify_nonce( yacht_rental_get_value_gp('nonce'), admin_url('admin-ajax.php') ) )
			die();
	
		$response = array('error'=>'');
		if (!($contact_email = yacht_rental_get_theme_option('contact_email')) && !($contact_email = yacht_rental_get_theme_option('admin_email'))) 
			$response['error'] = esc_html__('Unknown admin email!', 'trx_utils');
		else {
			$type = yacht_rental_substr($_REQUEST['type'], 0, 7);
			parse_str($_POST['data'], $post_data);

			if (in_array($type, array('form_1', 'form_2'))) {
				$user_name	= yacht_rental_strshort($post_data['username'],	100);
				$user_email	= yacht_rental_strshort($post_data['email'],	100);
				$user_subj	= yacht_rental_strshort($post_data['subject'],	100);
				$user_msg	= yacht_rental_strshort($post_data['message'],	yacht_rental_get_theme_option('message_maxlength_contacts'));
			...
			$mail = yacht_rental_get_theme_option('mail_function') == 'mail' ? 'mail' : 'wp_mail';
			if (is_email($contact_email) && !@$mail($contact_email, $subj, apply_filters('yacht_rental_filter_form_send_message', $msg))) {
...
if ( !function_exists( 'yacht_rental_sc_form_show_fields' ) ) {
	function yacht_rental_sc_form_show_fields($fields) {
...
-------------------------------------------------------------------- */
if ( !function_exists( 'yacht_rental_sc_form_reg_shortcodes' ) ) {
	function yacht_rental_sc_form_reg_shortcodes() {
	
		$pages = yacht_rental_get_list_pages(false);

		yacht_rental_sc_map("trx_form", array(
...
//				),
				"style" => array(
					...
					"divider" => true,
					"value" => 'form_custom',
					"options" => yacht_rental_get_sc_param('forms'),
					"type" => "checklist"
				), 
				"scheme" => array(
			...
			"children" => array(
				...
				"params" => array(
					"type" => array(
						...
						"type" => "checklist",
						"dir" => "horizontal",
						"options" => yacht_rental_get_sc_param('field_types')
					), 
					"name" => array(
					...
					"label_position" => array(
						...
						"type" => "checklist",
						"dir" => "horizontal",
						"options" => yacht_rental_get_sc_param('label_positions')
					), 
					"top" => yacht_rental_get_sc_param('top'),
					"bottom" => yacht_rental_get_sc_param('bottom'),
					"left" => yacht_rental_get_sc_param('left'),
...
-------------------------------------------------------------------- */
if ( !function_exists( 'yacht_rental_sc_form_reg_shortcodes_vc' ) ) {
	function yacht_rental_sc_form_reg_shortcodes_vc() {
		...
		vc_map( array(
			...
			"params" => array(
				array(
					...
					"class" => "",
					"std" => "form_custom",
					"value" => array_flip((array)yacht_rental_get_sc_param('forms')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "scheme",
...
//				),
		...
		vc_map( array(
			...
			"params" => array(
				array(
					...
					"admin_label" => true,
					"class" => "",
					"value" => array_flip((array)yacht_rental_get_sc_param('field_types')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "name",
				...
				array(
					...
					"class" => "",
					"value" => array_flip((array)yacht_rental_get_sc_param('label_positions')),
					"type" => "dropdown"
				),
				yacht_rental_get_vc_param('id'),
				yacht_rental_get_vc_param('class'),
		...
		class WPBakeryShortCode_Trx_Form extends Yacht_Rental_VC_ShortCodeCollection {}
		class WPBakeryShortCode_Trx_Form_Item extends Yacht_Rental_VC_ShortCodeItem {}
	}
}
?>