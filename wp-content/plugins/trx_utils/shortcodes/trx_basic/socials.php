...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('yacht_rental_sc_socials_theme_setup')) {
	add_action( 'yacht_rental_action_before_init_theme', 'yacht_rental_sc_socials_theme_setup' );
	function yacht_rental_sc_socials_theme_setup() {
		add_action('yacht_rental_action_shortcodes_list', 		'yacht_rental_sc_socials_reg_shortcodes');
		if (function_exists('yacht_rental_exists_visual_composer') && yacht_rental_exists_visual_composer())
			add_action('yacht_rental_action_shortcodes_list_vc','yacht_rental_sc_socials_reg_shortcodes_vc');
	}
}
...
if (!function_exists('yacht_rental_sc_socials')) {	
	function yacht_rental_sc_socials($atts, $content=null){	
		if (yacht_rental_in_shortcode_blogger()) return '';
		extract(yacht_rental_html_decode(shortcode_atts(array(
			// Individual params
			"size" => "small",		// tiny | small | medium | large
			"shape" => "round",	// round | square
			"type" => yacht_rental_get_theme_setting('socials_type'),	// icons | images
			"socials" => "",
			"custom" => "no",
			// Common params
			"id" => "",
		...
		yacht_rental_storage_set('sc_social_data', array(
			'icons' => false,
            'type' => $type
            )
        );
		if (!empty($socials)) {
			...
			for ($i=0; $i<count($allowed); $i++) {
				...
				if (!empty($s[1])) {
					$list[] = array(
						'icon'	=> $type=='images' ? yacht_rental_get_socials_url($s[0]) : 'icon-'.trim($s[0]),
						'url'	=> $s[1]
						);
				}
			}
			if (count($list) > 0) yacht_rental_storage_set_array('sc_social_data', 'icons', $list);
		} else if (yacht_rental_param_is_off($custom))
			$content = do_shortcode($content);
		if (yacht_rental_storage_get_array('sc_social_data', 'icons')===false) yacht_rental_storage_set_array('sc_social_data', 'icons', yacht_rental_get_custom_option('social_icons'));
		$output = yacht_rental_prepare_socials(yacht_rental_storage_get_array('sc_social_data', 'icons'));
		$output = $output
			? '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') 
				...
				. '</div>'
			: '';
		return apply_filters('yacht_rental_shortcode_output', $output, 'trx_socials', $atts, $content);
	}
	add_shortcode('trx_socials', 'yacht_rental_sc_socials');
}


if (!function_exists('yacht_rental_sc_social_item')) {	
	function yacht_rental_sc_social_item($atts, $content=null){	
		if (yacht_rental_in_shortcode_blogger()) return '';
		extract(yacht_rental_html_decode(shortcode_atts(array(
			// Individual params
			"name" => "",
		...
		if (!empty($name) && empty($icon)) {
			$type = yacht_rental_storage_get_array('sc_social_data', 'type');
			if ($type=='images') {
				if (file_exists(yacht_rental_get_socials_dir($name.'.png')))
					$icon = yacht_rental_get_socials_url($name.'.png');
			} else
				$icon = 'icon-'.esc_attr($name);
		}
		if (!empty($icon) && !empty($url)) {
			if (yacht_rental_storage_get_array('sc_social_data', 'icons')===false) yacht_rental_storage_set_array('sc_social_data', 'icons', array());
			yacht_rental_storage_set_array2('sc_social_data', 'icons', '', array(
				'icon' => $icon,
				'url' => $url
				)
			);
	...
	add_shortcode('trx_social_item', 'yacht_rental_sc_social_item');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'yacht_rental_sc_socials_reg_shortcodes' ) ) {
	function yacht_rental_sc_socials_reg_shortcodes() {
	
		yacht_rental_sc_map("trx_socials", array(
			...
			"params" => array(
				"type" => array(
					...
					"value" => yacht_rental_get_theme_setting('socials_type'),
					"options" => array(
...
//					"value" => "small",
//					"options" => yacht_rental_get_sc_param('sizes'),
//					"type" => "checklist"
//				), 
//				"shape" => array(
...
//					"value" => "square",
//					"options" => yacht_rental_get_sc_param('shapes'),
//					"type" => "checklist"
//				), 
				"socials" => array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'yacht_rental_sc_socials_reg_shortcodes_vc' ) ) {
	function yacht_rental_sc_socials_reg_shortcodes_vc() {
		...
		vc_map( array(
			...
			"params" => array_merge(array(
				array(
					...
					"class" => "",
					"std" => yacht_rental_get_theme_setting('socials_type'),
					"value" => array(
...
//					"class" => "",
//					"std" => "small",
//					"value" => array_flip(yacht_rental_get_sc_param('sizes')),
//					"type" => "dropdown"
//				),
//				array(
//					"param_name" => "shape",
...
//					"class" => "",
//					"std" => "square",
//					"value" => array_flip(yacht_rental_get_sc_param('shapes')),
//					"type" => "dropdown"
//				),
				array(
					"param_name" => "socials",
		...
		class WPBakeryShortCode_Trx_Socials extends Yacht_Rental_VC_ShortCodeCollection {}
		class WPBakeryShortCode_Trx_Social_Item extends Yacht_Rental_VC_ShortCodeSingle {}
	}
}
?>