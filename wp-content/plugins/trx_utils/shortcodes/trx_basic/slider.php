...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('yacht_rental_sc_slider_theme_setup')) {
	add_action( 'yacht_rental_action_before_init_theme', 'yacht_rental_sc_slider_theme_setup' );
	function yacht_rental_sc_slider_theme_setup() {
		add_action('yacht_rental_action_shortcodes_list', 		'yacht_rental_sc_slider_reg_shortcodes');
		if (function_exists('yacht_rental_exists_visual_composer') && yacht_rental_exists_visual_composer())
			add_action('yacht_rental_action_shortcodes_list_vc','yacht_rental_sc_slider_reg_shortcodes_vc');
	}
}
...
if (!function_exists('yacht_rental_sc_slider')) {	
	function yacht_rental_sc_slider($atts, $content=null){	
		...
		extract(yacht_rental_html_decode(shortcode_atts(array(
			...
			"slides_per_view" => 1,
			"titles" => "no",
			"descriptions" => yacht_rental_get_custom_option('slider_info_descriptions'),
			"links" => "no",
			"align" => "",
			"interval" => "",
			"date_format" => "",
		...
		if (!empty($height) && yacht_rental_param_is_on($autoheight)) $autoheight = "off";
		...
		yacht_rental_storage_set('sc_slider_data', array(
			'engine' => $engine,
            'width'  => yacht_rental_prepare_css_value($width),
            'height' => yacht_rental_prepare_css_value($height),
            'links'  => yacht_rental_param_is_on($links),
            'bg_image' => yacht_rental_get_theme_setting('slides_type')=='bg',
            'crop_image' => $crop
            )
        );
		...
		$class2 = yacht_rental_get_css_position_as_classes($top, $right, $bottom, $left);
		...
		$output = (in_array($pagination, array('full', 'over')) 
				...
				. ' class="sc_slider sc_slider_' . esc_attr($engine)
					...
					. (yacht_rental_param_is_on($controls) ? ' sc_slider_controls' : ' sc_slider_nocontrols')
					. (yacht_rental_param_is_on($pagination) ? ' sc_slider_pagination' : ' sc_slider_nopagination')
					. (yacht_rental_storage_get_array('sc_slider_data', 'bg_image') ? ' sc_slider_bg' : ' sc_slider_images')
					. (!in_array($pagination, array('full', 'over')) 
							? ($class2 ? ' '.esc_attr($class2) : '') . ($align!='' && $align!='none' ? ' align'.esc_attr($align) : '')
							: '')
					. '"'
				. (!in_array($pagination, array('full', 'over')) && !yacht_rental_param_is_off($animation) ? ' data-animation="'.esc_attr(yacht_rental_get_animation_classes($animation)).'"' : '')
		...
		if ($engine=='revo') {
			...
			$output .= '<div class="slides'
				...
				.($engine=='swiper' && yacht_rental_storage_get_array('sc_slider_data', 'bg_image') ? ' style="'.esc_attr($hs).'"' : '')
				.'>';
			...
			if (yacht_rental_param_is_on($custom) && $content) {
				...
				$args = yacht_rental_query_add_filters($args, 'thumbs');
				$args = yacht_rental_query_add_posts_and_cats($args, $ids, $post_type, $cat);
				...
				while ( $query->have_posts() ) { 
					...
					if (yacht_rental_param_is_on($crop)) {
						$post_attachment = yacht_rental_storage_get_array('sc_slider_data', 'bg_image')
							? yacht_rental_get_resized_image_url($post_attachment, !empty($width) && (float) $width.' ' == $width.' ' ? $width : null, !empty($height) && (float) $height.' ' == $height.' ' ? $height : null)
							: yacht_rental_get_resized_image_tag($post_attachment, !empty($width) && (float) $width.' ' == $width.' ' ? $width : null, !empty($height) && (float) $height.' ' == $height.' ' ? $height : null);
					} else if (!yacht_rental_storage_get_array('sc_slider_data', 'bg_image')) {
					...
					if (in_array($pagination, array('full', 'over'))) {
						$old_output = $output;
						$output = '';
						if (file_exists(yacht_rental_get_file_dir('templates/_parts/widgets-posts.php'))) {
					...
					$output .= '<div' 
						...
						. ' style="'
							. (yacht_rental_storage_get_array('sc_slider_data', 'bg_image') ? 'background-image:url(' . esc_url($post_attachment) . ');' : '') . ($ws) . ($hs)
							. '"'
						. '>' 
						. (yacht_rental_param_is_on($links) ? '<a href="'.esc_url($post_link).'" title="'.esc_attr($post_title).'">' : '')
						. (!yacht_rental_storage_get_array('sc_slider_data', 'bg_image') ? $post_attachment : '')
						;
					$caption = $engine=='swiper' ? '' : $caption;
					if (!yacht_rental_param_is_off($titles)) {
						$post_hover_bg  = yacht_rental_get_scheme_color('text_link');
						$post_bg = '';
						if ($post_hover_bg!='' && !yacht_rental_is_inherit_option($post_hover_bg)) {
							$rgb = yacht_rental_hex2rgb($post_hover_bg);
						...
						$post_descr = yacht_rental_get_post_excerpt();
						if (yacht_rental_get_custom_option("slider_info_category")=='yes') { // || empty($cat)) {
							// Get all post's categories
							$post_tax = yacht_rental_get_taxonomy_categories_by_post_type($post_type);
							if (!empty($post_tax)) {
								$post_terms = yacht_rental_get_terms_by_post_id(array('post_id'=>$post_id, 'taxonomy'=>$post_tax));
								if (!empty($post_terms[$post_tax])) {
									...
									if ($post_category!='') {
										$caption .= '<div class="sc_slider_category"'.(yacht_rental_substr($post_accent_color, 0, 1)=='#' ? ' style="background-color: '.esc_attr($post_accent_color).'"' : '').'><a href="'.esc_url($post_category_link).'">'.($post_category).'</a></div>';
									}
								}
							}
						}
						$output_reviews = '';
						if (yacht_rental_get_custom_option('show_reviews')=='yes' && yacht_rental_get_custom_option('slider_info_reviews')=='yes') {
							$avg_author = yacht_rental_reviews_marks_to_display(get_post_meta($post_id, yacht_rental_storage_get('options_prefix').'_reviews_avg'.((yacht_rental_get_theme_option('reviews_first')=='author' && $orderby != 'users_rating') || $orderby == 'author_rating' ? '' : '2'), true));
							if ($avg_author > 0) {
								$output_reviews .= '<div class="sc_slider_reviews post_rating reviews_summary blog_reviews' . (yacht_rental_get_custom_option("slider_info_category")=='yes' ? ' after_category' : '') . '">'
									. '<div class="criteria_summary criteria_row">' . trim(yacht_rental_reviews_get_summary_stars($avg_author, false, false, 5)) . '</div>'
									. '</div>';
							}
						}
						if (yacht_rental_get_custom_option("slider_info_category")=='yes') $caption .= $output_reviews;
						$caption .= '<h3 class="sc_slider_subtitle"><a href="'.esc_url($post_link).'">'.($post_title).'</a></h3>';
						if (yacht_rental_get_custom_option("slider_info_category")!='yes') $caption .= $output_reviews;
						if ($descriptions > 0) {
							$caption .= '<div class="sc_slider_descr">'.trim(yacht_rental_strshort($post_descr, $descriptions)).'</div>';
						}
						$caption .= '</div>';
					}
					$output .= ($engine=='swiper' ? $caption : '') . (yacht_rental_param_is_on($links) ? '</a>' : '' ) . '</div>';
				}
				wp_reset_postdata();
			}
		...
		return apply_filters('yacht_rental_shortcode_output', $output, 'trx_slider', $atts, $content);
	}
	add_shortcode('trx_slider', 'yacht_rental_sc_slider');
}


if (!function_exists('yacht_rental_sc_slider_item')) {	
	function yacht_rental_sc_slider_item($atts, $content=null) {
		if (yacht_rental_in_shortcode_blogger()) return '';
		extract(yacht_rental_html_decode(shortcode_atts( array(
			// Individual params
			"src" => "",
		...
		$engine = yacht_rental_storage_get_array('sc_slider_data', 'engine');
		$crop_image = yacht_rental_storage_get_array('sc_slider_data', 'crop_image');
		$bg_image = yacht_rental_storage_get_array('sc_slider_data', 'bg_image');
		$links = yacht_rental_storage_get_array('sc_slider_data', 'links');
		$width = yacht_rental_storage_get_array('sc_slider_data', 'width');
		$height = yacht_rental_storage_get_array('sc_slider_data', 'height');
	
		if ($src && yacht_rental_param_is_on($crop_image)) {
			$src = yacht_rental_storage_get_array('sc_slider_data', 'bg_image')
				? yacht_rental_get_resized_image_url($src, !empty($width) && yacht_rental_strpos($width, '%')===false ? $width : null, !empty($height) && yacht_rental_strpos($height, '%')===false ? $height : null)
				: yacht_rental_get_resized_image_tag($src, !empty($width) && yacht_rental_strpos($width, '%')===false ? $width : null, !empty($height) && yacht_rental_strpos($height, '%')===false ? $height : null);
		...
		$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '').' class="'.esc_attr($engine).'-slide' . (!empty($class) ? ' '.esc_attr($class) : '') . '"'
				. ($css ? ' style="'.esc_attr($css).'"' : '')
				.'>' 
				. ($src && yacht_rental_param_is_on($links) ? '<a href="'.esc_url($src).'">' : '')
				. ($src && !$bg_image ? $src : $content)
				. ($src && yacht_rental_param_is_on($links) ? '</a>' : '')
			. '</div>';
		return apply_filters('yacht_rental_shortcode_output', $output, 'trx_slider_item', $atts, $content);
	}
	add_shortcode('trx_slider_item', 'yacht_rental_sc_slider_item');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'yacht_rental_sc_slider_reg_shortcodes' ) ) {
	function yacht_rental_sc_slider_reg_shortcodes() {
	
		yacht_rental_sc_map("trx_slider", array(
			...
			"params" => array_merge(array(
				"engine" => array(
					...
					"value" => "swiper",
					"type" => "checklist",
					"options" => yacht_rental_get_sc_param('sliders')
				),
				"align" => array(
				...
				function_exists('yacht_rental_exists_revslider') && yacht_rental_exists_revslider() ? array(
				"alias" => array(
					...
					"value" => "",
					"type" => "select",
					"options" => yacht_rental_get_sc_param('revo_sliders')
				)) : array(), array(
				"cat" => array(
					...
					"style" => "list",
					"multiple" => true,
					"options" => yacht_rental_array_merge(array(0 => esc_html__('- Select category -', 'trx_utils')), yacht_rental_get_sc_param('categories'))
				),
				"count" => array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'yacht_rental_sc_slider_reg_shortcodes_vc' ) ) {
	function yacht_rental_sc_slider_reg_shortcodes_vc() {
		...
		vc_map( array(
			...
			"params" => array_merge(array(
				array(
					...
					"admin_label" => true,
					"class" => "",
					"value" => array_flip((array)yacht_rental_get_sc_param('sliders')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "align",
				...
				array(
					...
					"value" => array_flip((array)yacht_rental_array_merge(array('none' => esc_html__('- Select slider -', 'trx_utils')), yacht_rental_get_sc_param('revo_sliders'))),
					"type" => "dropdown"
				)) : array(), array(
				array(
					"param_name" => "cat",
					...
					"class" => "",
					"value" => array_flip((array)yacht_rental_array_merge(array(0 => esc_html__('- Select category -', 'trx_utils')), yacht_rental_get_sc_param('categories'))),
					"type" => "dropdown"
				),
				array(
					"param_name" => "count",
		...
		class WPBakeryShortCode_Trx_Slider extends Yacht_Rental_VC_ShortCodeCollection {}
		class WPBakeryShortCode_Trx_Slider_Item extends Yacht_Rental_VC_ShortCodeSingle {}
	}
}
?>