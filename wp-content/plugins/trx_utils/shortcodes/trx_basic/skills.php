...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('yacht_rental_sc_skills_theme_setup')) {
	add_action( 'yacht_rental_action_before_init_theme', 'yacht_rental_sc_skills_theme_setup' );
	function yacht_rental_sc_skills_theme_setup() {
		add_action('yacht_rental_action_shortcodes_list', 		'yacht_rental_sc_skills_reg_shortcodes');
		if (function_exists('yacht_rental_exists_visual_composer') && yacht_rental_exists_visual_composer())
			add_action('yacht_rental_action_shortcodes_list_vc','yacht_rental_sc_skills_reg_shortcodes_vc');
	}
}
...
if (!function_exists('yacht_rental_sc_skills')) {	
	function yacht_rental_sc_skills($atts, $content=null){	
		...
		yacht_rental_storage_set('sc_skills_data', array(
			'counter' => 0,
            'columns' => 0,
            'height'  => 0,
            'type'    => $type,
            'pie_compact' => yacht_rental_param_is_on($pie_compact) ? 'on' : 'off',
		...
		yacht_rental_enqueue_diagram($type);
		if ($type!='arc') {
			if ($layout=='' || ($layout=='columns' && $columns<1)) $layout = 'rows';
			if ($layout=='columns') yacht_rental_storage_set_array('sc_skills_data', 'columns', $columns);
			if ($type=='bar') {
		...
		if ($style) {
			...
			yacht_rental_storage_set_array('sc_skills_data', 'style', $style);
		}
		yacht_rental_storage_set_array('sc_skills_data', 'max', $max_value);
		yacht_rental_storage_set_array('sc_skills_data', 'dir', $dir);
		yacht_rental_storage_set_array('sc_skills_data', 'height', yacht_rental_prepare_css_value($height));
		...
		if (!yacht_rental_storage_empty('sc_skills_data', 'height') && (yacht_rental_storage_get_array('sc_skills_data', 'type') == 'arc' || (yacht_rental_storage_get_array('sc_skills_data', 'type') == 'pie' && yacht_rental_param_is_on(yacht_rental_storage_get_array('sc_skills_data', 'pie_compact')))))
			$css .= 'height: '.yacht_rental_storage_get_array('sc_skills_data', 'height');
		$content = do_shortcode($content);
		$output = '<div id="'.esc_attr($id).'"' 
					. ' class="sc_skills sc_skills_' . esc_attr($type) 
						. ($type=='bar' ? ' sc_skills_'.esc_attr($dir) : '') 
						. ($type=='pie' ? ' sc_skills_compact_'.esc_attr(yacht_rental_storage_get_array('sc_skills_data', 'pie_compact')) : '') 
				...
				. '>'
					...
					. ($type=='arc' 
						? ('<div class="sc_skills_legend">'.(yacht_rental_storage_get_array('sc_skills_data', 'legend')).'</div>'
							. '<div id="'.esc_attr($id).'_diagram" class="sc_skills_arc_canvas"></div>'
							. '<div class="sc_skills_data" style="display:none;">' . (yacht_rental_storage_get_array('sc_skills_data', 'data')) . '</div>'
						  )
						: '')
					. ($type=='pie' && yacht_rental_param_is_on(yacht_rental_storage_get_array('sc_skills_data', 'pie_compact'))
						? ('<div class="sc_skills_legend">'.(yacht_rental_storage_get_array('sc_skills_data', 'legend')).'</div>'
				...
				. '</div>';
		return apply_filters('yacht_rental_shortcode_output', $output, 'trx_skills', $atts, $content);
	}
	add_shortcode('trx_skills', 'yacht_rental_sc_skills');
}


if (!function_exists('yacht_rental_sc_skills_item')) {	
	function yacht_rental_sc_skills_item($atts, $content=null) {
		if (yacht_rental_in_shortcode_blogger()) return '';
		extract(yacht_rental_html_decode(shortcode_atts( array(
			// Individual params
			"title" => "",
		...
		yacht_rental_storage_inc_array('sc_skills_data', 'counter');
		$ed = yacht_rental_substr($value, -1)=='%' ? '%' : '';
		$value = str_replace('%', '', $value);
		if (yacht_rental_storage_get_array('sc_skills_data', 'max') < $value) yacht_rental_storage_set_array('sc_skills_data', 'max', $value);
		$percent = round($value / yacht_rental_storage_get_array('sc_skills_data', 'max') * 100);
		$start = 0;
		$stop = $value;
		$steps = 100;
		$step = max(1, round(yacht_rental_storage_get_array('sc_skills_data', 'max')/$steps));
		$speed = mt_rand(10,40);
		$animation = round(($stop - $start) / $step * $speed);
		$title_block = '<div class="sc_skills_info"><div class="sc_skills_label">' . ($title) . '</div></div>';
		$old_color = $color;
		if (empty($color)) $color = yacht_rental_storage_get_array('sc_skills_data', 'color');
		if (empty($color)) $color = yacht_rental_get_scheme_color('text_link', $color);
		if (empty($bg_color)) $bg_color = yacht_rental_storage_get_array('sc_skills_data', 'bg_color');
		if (empty($bg_color)) $bg_color = yacht_rental_get_scheme_color('bg_color', $bg_color);
		if (empty($border_color)) $border_color = yacht_rental_storage_get_array('sc_skills_data', 'border_color');
		if (empty($border_color)) $border_color = yacht_rental_get_scheme_color('bd_color', $border_color);;
		if (empty($style)) $style = yacht_rental_storage_get_array('sc_skills_data', 'style');
		$style = max(1, min(4, $style));
		$output = '';
		if (yacht_rental_storage_get_array('sc_skills_data', 'type') == 'arc' || (yacht_rental_storage_get_array('sc_skills_data', 'type') == 'pie' && yacht_rental_param_is_on(yacht_rental_storage_get_array('sc_skills_data', 'pie_compact')))) {
			if (yacht_rental_storage_get_array('sc_skills_data', 'type') == 'arc' && empty($old_color)) {
				$rgb = yacht_rental_hex2rgb($color);
				$color = 'rgba('.(int)$rgb['r'].','.(int)$rgb['g'].','.(int)$rgb['b'].','.(1 - 0.1*(yacht_rental_storage_get_array('sc_skills_data', 'counter')-1)).')';
			}
			yacht_rental_storage_concat_array('sc_skills_data', 'legend', 
			...
			yacht_rental_storage_concat_array('sc_skills_data', 'data', 
				'<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
					. ' class="'.esc_attr(yacht_rental_storage_get_array('sc_skills_data', 'type')).'"'
					. (yacht_rental_storage_get_array('sc_skills_data', 'type')=='pie'
						? ( ' data-start="'.esc_attr($start).'"'
							. ' data-stop="'.esc_attr($stop).'"'
							. ' data-step="'.esc_attr($step).'"'
							. ' data-steps="'.esc_attr($steps).'"'
							. ' data-max="'.esc_attr(yacht_rental_storage_get_array('sc_skills_data', 'max')).'"'
							...
							. ' data-cutout="'.esc_attr(yacht_rental_storage_get_array('sc_skills_data', 'pie_cutout')).'"'
			...
			$output .= (yacht_rental_storage_get_array('sc_skills_data', 'columns') > 0 
							? '<div class="sc_skills_column column-1_'.esc_attr(yacht_rental_storage_get_array('sc_skills_data', 'columns')).'">' 
							: '')
					. (yacht_rental_storage_get_array('sc_skills_data', 'type')=='bar' && yacht_rental_storage_get_array('sc_skills_data', 'dir')=='horizontal' ? $title_block : '')
					. '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') 
						. ' class="sc_skills_item' . ($style ? ' sc_skills_style_'.esc_attr($style) : '') 
							. (!empty($class) ? ' '.esc_attr($class) : '')
							. (yacht_rental_storage_get_array('sc_skills_data', 'counter') % 2 == 1 ? ' odd' : ' even') 
							. (yacht_rental_storage_get_array('sc_skills_data', 'counter') == 1 ? ' first' : '') 
							. '"'
						. (yacht_rental_storage_get_array('sc_skills_data', 'height') !='' || $css 
							? ' style="' 
								. (yacht_rental_storage_get_array('sc_skills_data', 'height') !='' 
										? 'height: '.esc_attr(yacht_rental_storage_get_array('sc_skills_data', 'height')).';' 
										: '') 
								. ($css) 
								. '"' 
							: '')
			...
			if (in_array(yacht_rental_storage_get_array('sc_skills_data', 'type'), array('bar', 'counter'))) {
				$output .= '<div class="sc_skills_count"' . (yacht_rental_storage_get_array('sc_skills_data', 'type')=='bar' && $color ? ' style="background-color:' . esc_attr($color) . '; border-color:' . esc_attr($color) . '"' : '') . '>'
						. '</div>'
			...
			} else if (yacht_rental_storage_get_array('sc_skills_data', 'type')=='pie') {
			...
			$output .= 
					  (yacht_rental_storage_get_array('sc_skills_data', 'type')=='counter' ? $title_block : '')
					. '</div>'
					. (yacht_rental_storage_get_array('sc_skills_data', 'type')=='bar' && yacht_rental_storage_get_array('sc_skills_data', 'dir')=='vertical' || yacht_rental_storage_get_array('sc_skills_data', 'type') == 'pie' ? $title_block : '')
					. (yacht_rental_storage_get_array('sc_skills_data', 'columns') > 0 ? '</div>' : '');
		}
		return apply_filters('yacht_rental_shortcode_output', $output, 'trx_skills_item', $atts, $content);
	}
	add_shortcode('trx_skills_item', 'yacht_rental_sc_skills_item');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'yacht_rental_sc_skills_reg_shortcodes' ) ) {
	function yacht_rental_sc_skills_reg_shortcodes() {
	
		yacht_rental_sc_map("trx_skills", array(
			...
			"params" => array(
				...
				"style" => array(
					...
					"value" => 1,
					"options" => yacht_rental_get_list_styles(1, 4),
					"type" => "checklist"
				), 
...
-------------------------------------------------------------------- */
if ( !function_exists( 'yacht_rental_sc_skills_reg_shortcodes_vc' ) ) {
	function yacht_rental_sc_skills_reg_shortcodes_vc() {
		...
		class WPBakeryShortCode_Trx_Skills extends Yacht_Rental_VC_ShortCodeCollection {}
		class WPBakeryShortCode_Trx_Skills_Item extends Yacht_Rental_VC_ShortCodeSingle {}
	}
}
?>