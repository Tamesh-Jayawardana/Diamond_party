...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('yacht_rental_sc_call_to_action_theme_setup')) {
	add_action( 'yacht_rental_action_before_init_theme', 'yacht_rental_sc_call_to_action_theme_setup' );
	function yacht_rental_sc_call_to_action_theme_setup() {
		add_action('yacht_rental_action_shortcodes_list', 		'yacht_rental_sc_call_to_action_reg_shortcodes');
		if (function_exists('yacht_rental_exists_visual_composer') && yacht_rental_exists_visual_composer())
			add_action('yacht_rental_action_shortcodes_list_vc','yacht_rental_sc_call_to_action_reg_shortcodes_vc');
	}
}
...
if (!function_exists('yacht_rental_sc_call_to_action')) {	
	function yacht_rental_sc_call_to_action($atts, $content=null){	
		...
		if (!empty($image)) {
			$thumb_sizes = yacht_rental_get_thumb_sizes(array('layout' => 'excerpt'));
			$image = !empty($video) 
				? yacht_rental_get_resized_image_url($image, $thumb_sizes['w'], $thumb_sizes['h']) 
				: yacht_rental_get_resized_image_tag($image, $thumb_sizes['w'], $thumb_sizes['h']);
		}
	
		if (!empty($video)) {
			$video = '<video' . ($id ? ' id="' . esc_attr($id.'_video') . '"' : '') 
				. ' class="sc_video"'
				. ' src="' . esc_url(yacht_rental_get_video_player_url($video)) . '"'
				...
				. '>'
				. '</video>';
			if (yacht_rental_get_custom_option('substitute_video')=='no') {
				$video = yacht_rental_get_video_frame($video, $image, '', '');
			} else {
		...
		$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') 
				. ' class="sc_call_to_action'
					. (yacht_rental_param_is_on($accent) ? ' sc_call_to_action_accented' : '')
			...
			. '>'
				...
				. '<div class="sc_call_to_action_info'.($need_columns ? ' column-'.esc_attr($need_columns-1).'_'.esc_attr($need_columns) : '').'">'
					. (!empty($subtitle) ? '<h6 class="sc_call_to_action_subtitle sc_item_subtitle">' . trim(yacht_rental_strmacros($subtitle)) . '</h6>' : '')
					. (!empty($title) ? '<h2 class="sc_call_to_action_title sc_item_title' . (empty($description) ? ' sc_item_title_without_descr' : ' sc_item_title_without_descr') . '">' . trim(yacht_rental_strmacros($title)) . '</h2>' : '')
					. (!empty($description) ? '<div class="sc_call_to_action_descr sc_item_descr">' . trim(yacht_rental_strmacros($description)) . '</div>' : '')
					. ($style==1 ? $buttons : '')
				. '</div>'
				. ($style==2 && $align!='right' ? $buttons : '')
				. ($align=='right' ? $featured : '')
		...
		return apply_filters('yacht_rental_shortcode_output', $output, 'trx_call_to_action', $atts, $content);
	}
	add_shortcode('trx_call_to_action', 'yacht_rental_sc_call_to_action');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'yacht_rental_sc_call_to_action_reg_shortcodes' ) ) {
	function yacht_rental_sc_call_to_action_reg_shortcodes() {
	
		yacht_rental_sc_map("trx_call_to_action", array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'yacht_rental_sc_call_to_action_reg_shortcodes_vc' ) ) {
	function yacht_rental_sc_call_to_action_reg_shortcodes_vc() {
		...
		vc_map( array(
			...
			"params" => array(
				array(
					...
					"class" => "",
					"admin_label" => true,
					"value" => array_flip((array)yacht_rental_get_list_styles(1, 2)),
					"type" => "dropdown"
				),
				array(
					"param_name" => "align",
		...
		class WPBakeryShortCode_Trx_Call_To_Action extends Yacht_Rental_VC_ShortCodeCollection {}
	}
}
?>