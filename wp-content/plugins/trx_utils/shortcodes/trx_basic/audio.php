...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('yacht_rental_sc_audio_theme_setup')) {
	add_action( 'yacht_rental_action_before_init_theme', 'yacht_rental_sc_audio_theme_setup' );
	function yacht_rental_sc_audio_theme_setup() {
		add_action('yacht_rental_action_shortcodes_list', 		'yacht_rental_sc_audio_reg_shortcodes');
		if (function_exists('yacht_rental_exists_visual_composer') && yacht_rental_exists_visual_composer())
			add_action('yacht_rental_action_shortcodes_list_vc','yacht_rental_sc_audio_reg_shortcodes_vc');
	}
}
...
if (!function_exists('yacht_rental_sc_audio')) {	
	function yacht_rental_sc_audio($atts, $content = null) {
		...
		$audio = '<audio'
			...
			. ' src="'.esc_url($src).'"'
			. (yacht_rental_param_is_on($controls) ? ' controls="controls"' : '')
			. (yacht_rental_param_is_on($autoplay) && is_single() ? ' autoplay="autoplay"' : '')
			. ' width="'.esc_attr($width).'" height="'.esc_attr($height).'"'
			. ($css!='' ? ' style="'.esc_attr($css).'"' : '')
			. ($data)
			. '></audio>';
		if ( yacht_rental_get_custom_option('substitute_audio')=='no') {
			if (yacht_rental_param_is_on($frame)) {
				$audio = yacht_rental_get_audio_frame($audio, $image, $s);
			}
		} else {
			if ((isset($_GET['vc_editable']) && $_GET['vc_editable']=='true') && (isset($_POST['action']) && $_POST['action']=='vc_load_shortcode')) {
				$audio = yacht_rental_substitute_audio($audio, false);
			}
		}
		if (yacht_rental_get_theme_option('use_mediaelement')=='yes')
			wp_enqueue_script('wp-mediaelement');
		return apply_filters('yacht_rental_shortcode_output', $audio, 'trx_audio', $atts, $content);
	}
	add_shortcode("trx_audio", "yacht_rental_sc_audio");
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'yacht_rental_sc_audio_reg_shortcodes' ) ) {
	function yacht_rental_sc_audio_reg_shortcodes() {
	
		yacht_rental_sc_map("trx_audio", array(
			...
			"params" => array(
				...
				"controls" => array(
					...
					"value" => "show",
					"type" => "switch",
					"options" => yacht_rental_get_sc_param('show_hide')
				),
				"autoplay" => array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'yacht_rental_sc_audio_reg_shortcodes_vc' ) ) {
	function yacht_rental_sc_audio_reg_shortcodes_vc() {
		...
		class WPBakeryShortCode_Trx_Audio extends Yacht_Rental_VC_ShortCodeSingle {}
	}
}
?>