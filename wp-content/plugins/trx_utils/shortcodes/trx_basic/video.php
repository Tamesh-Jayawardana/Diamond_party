...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('yacht_rental_sc_video_theme_setup')) {
	add_action( 'yacht_rental_action_before_init_theme', 'yacht_rental_sc_video_theme_setup' );
	function yacht_rental_sc_video_theme_setup() {
		add_action('yacht_rental_action_shortcodes_list', 		'yacht_rental_sc_video_reg_shortcodes');
		if (function_exists('yacht_rental_exists_visual_composer') && yacht_rental_exists_visual_composer())
			add_action('yacht_rental_action_shortcodes_list_vc','yacht_rental_sc_video_reg_shortcodes_vc');
	}
}
...
if (!function_exists('yacht_rental_sc_video')) {	
	function yacht_rental_sc_video($atts, $content = null) {
		...
		if (empty($height) && empty($width)) {
			$width='100%';
			if (yacht_rental_param_is_off(yacht_rental_get_custom_option('substitute_video'))) $height="400";
		}
		$ed = yacht_rental_substr($width, -1);
		...
		$css_bg = yacht_rental_get_css_paddings_from_values($bg_top, $bg_right, $bg_bottom, $bg_left);
	
		if ($src=='' && $url=='' && isset($atts[0])) {
			$src = $atts[0];
		}
		$url = $src!='' ? $src : $url;
		if ($image!='' && yacht_rental_param_is_off($image))
			$image = '';
		else {
			if (yacht_rental_param_is_on($autoplay) && is_singular() && !yacht_rental_storage_get('blog_streampage'))
				$image = '';
			else {
				if ($image > 0) {
				...
				if ($bg_image) {
					...
					if (!is_single() || !empty($image)) $image = yacht_rental_get_resized_image_url(empty($image) ? get_the_ID() : $image, $thumb_sizes['w'], $thumb_sizes['h'], null, false, false, false);
				} else
					if (!is_single() || !empty($image)) $image = yacht_rental_get_resized_image_url(empty($image) ? get_the_ID() : $image, $ed!='%' ? $width : null, $height);
				if (empty($image) && (!is_singular() || yacht_rental_storage_get('blog_streampage')))	// || yacht_rental_param_is_off($autoplay)))
					$image = yacht_rental_get_video_cover_image($url);
			}
		}
		if ($bg_image > 0) {
		...
		$url = yacht_rental_get_video_player_url($src!='' ? $src : $url);
		...
		$video = '<video' . ($id ? ' id="' . esc_attr($id) . '"' : '') 
			...
			. (($image && yacht_rental_param_is_on(yacht_rental_get_custom_option('substitute_video'))) || (yacht_rental_param_is_on($autoplay) && is_singular() && !yacht_rental_storage_get('blog_streampage')) ? ' autoplay="autoplay"' : '') 
			. ' controls="controls" loop="loop"'
			. '>'
			. '</video>';
		if (yacht_rental_param_is_off(yacht_rental_get_custom_option('substitute_video'))) {
			if (yacht_rental_param_is_on($frame)) $video = yacht_rental_get_video_frame($video, $image, $css, $css_bg);
		} else {
			if ((isset($_GET['vc_editable']) && $_GET['vc_editable']=='true') && (isset($_POST['action']) && $_POST['action']=='vc_load_shortcode')) {
				$video = yacht_rental_substitute_video($video, $width, $height, false);
			}
		}
		if (yacht_rental_get_theme_option('use_mediaelement')=='yes')
			wp_enqueue_script('wp-mediaelement');
		return apply_filters('yacht_rental_shortcode_output', $video, 'trx_video', $atts, $content);
	}
	add_shortcode("trx_video", "yacht_rental_sc_video");
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'yacht_rental_sc_video_reg_shortcodes' ) ) {
	function yacht_rental_sc_video_reg_shortcodes() {
	
		yacht_rental_sc_map("trx_video", array(
			...
			"params" => array(
				...
				"autoplay" => array(
					...
					"value" => "off",
					"type" => "switch",
					"options" => yacht_rental_get_sc_param('on_off')
				),
				"align" => array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'yacht_rental_sc_video_reg_shortcodes_vc' ) ) {
	function yacht_rental_sc_video_reg_shortcodes_vc() {
		...
		class WPBakeryShortCode_Trx_Video extends Yacht_Rental_VC_ShortCodeSingle {}
	}
}
?>