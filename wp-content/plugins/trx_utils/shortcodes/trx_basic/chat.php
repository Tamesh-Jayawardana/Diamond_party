...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('yacht_rental_sc_chat_theme_setup')) {
	add_action( 'yacht_rental_action_before_init_theme', 'yacht_rental_sc_chat_theme_setup' );
	function yacht_rental_sc_chat_theme_setup() {
		add_action('yacht_rental_action_shortcodes_list', 		'yacht_rental_sc_chat_reg_shortcodes');
		if (function_exists('yacht_rental_exists_visual_composer') && yacht_rental_exists_visual_composer())
			add_action('yacht_rental_action_shortcodes_list_vc','yacht_rental_sc_chat_reg_shortcodes_vc');
	}
}
...
if (!function_exists('yacht_rental_sc_chat')) {	
	function yacht_rental_sc_chat($atts, $content=null){	
		...
		if (!empty($photo)) {
			...
			$photo = yacht_rental_get_resized_image_tag($photo, 75, 75);
		}
		...
		$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') 
				...
				. '>'
					...
					. '</div>'
				. '</div>';
		return apply_filters('yacht_rental_shortcode_output', $output, 'trx_chat', $atts, $content);
	}
	add_shortcode('trx_chat', 'yacht_rental_sc_chat');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'yacht_rental_sc_chat_reg_shortcodes' ) ) {
	function yacht_rental_sc_chat_reg_shortcodes() {
	
		yacht_rental_sc_map("trx_chat", array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'yacht_rental_sc_chat_reg_shortcodes_vc' ) ) {
	function yacht_rental_sc_chat_reg_shortcodes_vc() {
		...
		class WPBakeryShortCode_Trx_Chat extends Yacht_Rental_VC_ShortCodeContainer {}
	}
}
?>