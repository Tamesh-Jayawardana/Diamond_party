...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('yacht_rental_sc_image_theme_setup')) {
	add_action( 'yacht_rental_action_before_init_theme', 'yacht_rental_sc_image_theme_setup' );
	function yacht_rental_sc_image_theme_setup() {
		add_action('yacht_rental_action_shortcodes_list', 		'yacht_rental_sc_image_reg_shortcodes');
		if (function_exists('yacht_rental_exists_visual_composer') && yacht_rental_exists_visual_composer())
			add_action('yacht_rental_action_shortcodes_list_vc','yacht_rental_sc_image_reg_shortcodes_vc');
	}
}
...
if (!function_exists('yacht_rental_sc_image')) {	
	function yacht_rental_sc_image($atts, $content=null){	
		...
		if (!empty($width) || !empty($height)) {
			...
			if ($w || $h) $src = yacht_rental_get_resized_image_url($src, $w, $h);
		}
		if (trim($link)) yacht_rental_enqueue_popup();
		$output = empty($src) ? '' : ('<figure' . ($id ? ' id="'.esc_attr($id).'"' : '') 
			...
			. '</figure>');
		return apply_filters('yacht_rental_shortcode_output', $output, 'trx_image', $atts, $content);
	}
	add_shortcode('trx_image', 'yacht_rental_sc_image');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'yacht_rental_sc_image_reg_shortcodes' ) ) {
	function yacht_rental_sc_image_reg_shortcodes() {
	
		yacht_rental_sc_map("trx_image", array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'yacht_rental_sc_image_reg_shortcodes_vc' ) ) {
	function yacht_rental_sc_image_reg_shortcodes_vc() {
		...
		class WPBakeryShortCode_Trx_Image extends Yacht_Rental_VC_ShortCodeSingle {}
	}
}
?>