...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('yacht_rental_sc_title_theme_setup')) {
	add_action( 'yacht_rental_action_before_init_theme', 'yacht_rental_sc_title_theme_setup' );
	function yacht_rental_sc_title_theme_setup() {
		add_action('yacht_rental_action_shortcodes_list', 		'yacht_rental_sc_title_reg_shortcodes');
		if (function_exists('yacht_rental_exists_visual_composer') && yacht_rental_exists_visual_composer())
			add_action('yacht_rental_action_shortcodes_list_vc','yacht_rental_sc_title_reg_shortcodes_vc');
	}
}
...
if (!function_exists('yacht_rental_sc_title')) {	
	function yacht_rental_sc_title($atts, $content=null){	
		...
		$css .= yacht_rental_get_css_dimensions_from_values($width)
			.($align && $align!='none' && !yacht_rental_param_is_inherit($align) ? 'text-align:' . esc_attr($align) .';' : '')
		...
		$pic = $style!='iconed' 
			...
			: '<span class="sc_title_icon sc_title_icon_'.esc_attr($position).'  sc_title_icon_'.esc_attr($image_size).($icon!='' && $icon!='none' ? ' '.esc_attr($icon) : '').'"'.'>'
				...
				.(empty($picture) && $image && $image!='none' ? '<img src="'.esc_url(yacht_rental_strpos($image, 'http')===0 ? $image : yacht_rental_get_file_url('images/icons/'.($image).'.png')).'" alt="" />' : '')
				.'</span>';
		$output = '<h' . esc_attr($type) . ($id ? ' id="'.esc_attr($id).'"' : '')
				. ' class="sc_title sc_title_'.esc_attr($style)
					.($align && $align!='none' && !yacht_rental_param_is_inherit($align) ? ' sc_align_' . esc_attr($align) : '')
				...
				. '</h' . esc_attr($type) . '>';
		return apply_filters('yacht_rental_shortcode_output', $output, 'trx_title', $atts, $content);
	}
	add_shortcode('trx_title', 'yacht_rental_sc_title');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'yacht_rental_sc_title_reg_shortcodes' ) ) {
	function yacht_rental_sc_title_reg_shortcodes() {
	
		yacht_rental_sc_map("trx_title", array(
			...
			"params" => array(
				...
				"image" => array(
					...
					"type" => "images",
					"size" => "small",
					"options" => yacht_rental_get_sc_param('images')
				),
				"picture" => array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'yacht_rental_sc_title_reg_shortcodes_vc' ) ) {
	function yacht_rental_sc_title_reg_shortcodes_vc() {
		...
		vc_map( array(
			...
			"params" => array(
				...
				array(
					...
					"value" => yacht_rental_get_sc_param('images'),
					"type" => "dropdown"
				),
				array(
					"param_name" => "picture",
		...
		class WPBakeryShortCode_Trx_Title extends Yacht_Rental_VC_ShortCodeSingle {}
	}
}
?>