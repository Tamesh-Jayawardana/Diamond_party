...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('yacht_rental_sc_list_theme_setup')) {
	add_action( 'yacht_rental_action_before_init_theme', 'yacht_rental_sc_list_theme_setup' );
	function yacht_rental_sc_list_theme_setup() {
		add_action('yacht_rental_action_shortcodes_list', 		'yacht_rental_sc_list_reg_shortcodes');
		if (function_exists('yacht_rental_exists_visual_composer') && yacht_rental_exists_visual_composer())
			add_action('yacht_rental_action_shortcodes_list_vc','yacht_rental_sc_list_reg_shortcodes_vc');
	}
}
...
if (!function_exists('yacht_rental_sc_list')) {	
	function yacht_rental_sc_list($atts, $content=null){	
		...
		yacht_rental_storage_set('sc_list_data', array(
			'counter' => 0,
            'icon' => empty($icon) || yacht_rental_param_is_inherit($icon) ? "icon-right" : $icon,
		...
		$output = '<' . ($style=='ol' ? 'ol' : 'ul')
				...
				. '</' .($style=='ol' ? 'ol' : 'ul') . '>';
		return apply_filters('yacht_rental_shortcode_output', $output, 'trx_list', $atts, $content);
	}
	add_shortcode('trx_list', 'yacht_rental_sc_list');
}


if (!function_exists('yacht_rental_sc_list_item')) {	
	function yacht_rental_sc_list_item($atts, $content=null) {
		if (yacht_rental_in_shortcode_blogger()) return '';
		extract(yacht_rental_html_decode(shortcode_atts( array(
			// Individual params
			"color" => "",
		...
		yacht_rental_storage_inc_array('sc_list_data', 'counter');
		$css .= $color !== '' ? 'color:' . esc_attr($color) .';' : '';
		if (trim($icon) == '' || yacht_rental_param_is_inherit($icon)) $icon = yacht_rental_storage_get_array('sc_list_data', 'icon');
		if (trim($color) == '' || yacht_rental_param_is_inherit($icon_color)) $icon_color = yacht_rental_storage_get_array('sc_list_data', 'icon_color');
		$content = do_shortcode($content);
		if (empty($content)) $content = $title;
		$output = '<li' . ($id ? ' id="'.esc_attr($id).'"' : '') 
			. ' class="sc_list_item' 
			. (!empty($class) ? ' '.esc_attr($class) : '')
			. (yacht_rental_storage_get_array('sc_list_data', 'counter') % 2 == 1 ? ' odd' : ' even') 
			. (yacht_rental_storage_get_array('sc_list_data', 'counter') == 1 ? ' first' : '')  
			. '"' 
			. ($css!='' ? ' style="'.esc_attr($css).'"' : '')
			. ($title ? ' title="'.esc_attr($title).'"' : '') 
			. '>' 
			. (!empty($link) ? '<a href="'.esc_url($link).'"' . (!empty($target) ? ' target="'.esc_attr($target).'"' : '') . '>' : '')
			. (yacht_rental_storage_get_array('sc_list_data', 'style')=='iconed' && $icon!='' ? '<span class="sc_list_icon '.esc_attr($icon).'"'.($icon_color !== '' ? ' style="color:'.esc_attr($icon_color).';"' : '').'></span>' : '')
			. trim($content)
			. (!empty($link) ? '</a>': '')
			. '</li>';
		return apply_filters('yacht_rental_shortcode_output', $output, 'trx_list_item', $atts, $content);
	}
	add_shortcode('trx_list_item', 'yacht_rental_sc_list_item');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'yacht_rental_sc_list_reg_shortcodes' ) ) {
	function yacht_rental_sc_list_reg_shortcodes() {
	
		yacht_rental_sc_map("trx_list", array(
			...
			"params" => array(
				"style" => array(
					...
					"value" => "ul",
					"type" => "checklist",
					"options" => yacht_rental_get_sc_param('list_styles')
				), 
				"color" => array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'yacht_rental_sc_list_reg_shortcodes_vc' ) ) {
	function yacht_rental_sc_list_reg_shortcodes_vc() {
		...
		vc_map( array(
			...
			"params" => array(
				array(
					...
					"class" => "",
					"admin_label" => true,
					"value" => array_flip((array)yacht_rental_get_sc_param('list_styles')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "color",
		...
		class WPBakeryShortCode_Trx_List extends Yacht_Rental_VC_ShortCodeCollection {}
		class WPBakeryShortCode_Trx_List_Item extends Yacht_Rental_VC_ShortCodeContainer {}
	}
}
?>