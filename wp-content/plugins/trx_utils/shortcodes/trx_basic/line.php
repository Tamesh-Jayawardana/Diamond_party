...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('yacht_rental_sc_line_theme_setup')) {
	add_action( 'yacht_rental_action_before_init_theme', 'yacht_rental_sc_line_theme_setup' );
	function yacht_rental_sc_line_theme_setup() {
		add_action('yacht_rental_action_shortcodes_list', 		'yacht_rental_sc_line_reg_shortcodes');
		if (function_exists('yacht_rental_exists_visual_composer') && yacht_rental_exists_visual_composer())
			add_action('yacht_rental_action_shortcodes_list_vc','yacht_rental_sc_line_reg_shortcodes_vc');
	}
}
...
if (!function_exists('yacht_rental_sc_line')) {	
	function yacht_rental_sc_line($atts, $content=null){	
		...
		if ($style=='image' && !empty($image)) {
			...
			$attr = yacht_rental_getimagesize($image);
		...
		$css .= yacht_rental_get_css_dimensions_from_values($width, $block_height)
			. ($style=='image' && !empty($image)
				? ( 'background-image: url(' . esc_url($image) . ');'
					. (yacht_rental_param_is_on($repeat) ? 'background-repeat: repeat-x;' : '')
					)
				: ( ($height !='' ? 'border-'.esc_attr($border_pos).'-width:' . esc_attr(yacht_rental_prepare_css_value($height)) . ';' : '')
		...
		$output = '<div' . ($id ? ' id="'.esc_attr($id) . '"' : '') 
				...
				. '</div>';
		return apply_filters('yacht_rental_shortcode_output', $output, 'trx_line', $atts, $content);
	}
	add_shortcode('trx_line', 'yacht_rental_sc_line');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'yacht_rental_sc_line_reg_shortcodes' ) ) {
	function yacht_rental_sc_line_reg_shortcodes() {
	
		yacht_rental_sc_map("trx_line", array(
			...
			"params" => array(
				"style" => array(
					...
					"value" => "solid",
					"dir" => "horizontal",
					"options" => yacht_rental_get_list_line_styles(),
					"type" => "checklist"
				),
				"image" => array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'yacht_rental_sc_line_reg_shortcodes_vc' ) ) {
	function yacht_rental_sc_line_reg_shortcodes_vc() {
		...
		vc_map( array(
			...
			"params" => array(
				array(
					...
					"class" => "",
					"std" => "solid",
					"value" => array_flip((array)yacht_rental_get_list_line_styles()),
					"type" => "dropdown"
				),
				array(
					"param_name" => "image",
				...
				array(
					...
					"class" => "",
					"std" => "center center",
					"value" => array_flip((array)yacht_rental_get_list_bg_image_positions()),
					"type" => "dropdown"
				),
		...
		class WPBakeryShortCode_Trx_Line extends Yacht_Rental_VC_ShortCodeSingle {}
	}
}
?>