...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('yacht_rental_sc_columns_theme_setup')) {
	add_action( 'yacht_rental_action_before_init_theme', 'yacht_rental_sc_columns_theme_setup' );
	function yacht_rental_sc_columns_theme_setup() {
		add_action('yacht_rental_action_shortcodes_list', 		'yacht_rental_sc_columns_reg_shortcodes');
		if (function_exists('yacht_rental_exists_visual_composer') && yacht_rental_exists_visual_composer())
			add_action('yacht_rental_action_shortcodes_list_vc','yacht_rental_sc_columns_reg_shortcodes_vc');
	}
}
...
if (!function_exists('yacht_rental_sc_columns')) {	
	function yacht_rental_sc_columns($atts, $content=null){	
		...
		yacht_rental_storage_set('sc_columns_data', array(
			'counter' => 1,
			'equal_selector' => '',
            'after_span2' => false,
            'after_span3' => false,
		...
		$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') 
				. ' class="columns_wrap sc_columns'
					. ' columns_' . (yacht_rental_param_is_on($fluid) ? 'fluid' : 'nofluid') 
					. (!empty($margins) && yacht_rental_param_is_off($margins) ? ' no_margins' : '') 
					. ' sc_columns_count_' . esc_attr($count)
					. (!empty($class) ? ' '.esc_attr($class) : '') 
				. '"'
				. ($css!='' ? ' style="'.esc_attr($css).'"' : '')
				. (!yacht_rental_param_is_off($equalheight) ? ' data-equal-height="'.esc_attr(yacht_rental_storage_get_array('sc_columns_data', 'equal_selector')).'"' : '')
				. (!yacht_rental_param_is_off($animation) ? ' data-animation="'.esc_attr(yacht_rental_get_animation_classes($animation)).'"' : '')
				. '>'
					. trim($content)
				. '</div>';
		return apply_filters('yacht_rental_shortcode_output', $output, 'trx_columns', $atts, $content);
	}
	add_shortcode('trx_columns', 'yacht_rental_sc_columns');
}


if (!function_exists('yacht_rental_sc_column_item')) {	
	function yacht_rental_sc_column_item($atts, $content=null) {
		if (yacht_rental_in_shortcode_blogger()) return '';
		extract(yacht_rental_html_decode(shortcode_atts( array(
			// Individual params
			"span" => "1",
		...
		$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') . ' class="column-'.($span > 1 ? esc_attr($span) : 1).'_'.esc_attr(yacht_rental_storage_get_array('sc_columns_data', 'count')).' sc_column_item sc_column_item_'.esc_attr(yacht_rental_storage_get_array('sc_columns_data', 'counter')) 
					. (!empty($class) ? ' '.esc_attr($class) : '')
					. (yacht_rental_storage_get_array('sc_columns_data', 'counter') % 2 == 1 ? ' odd' : ' even') 
					. (yacht_rental_storage_get_array('sc_columns_data', 'counter') == 1 ? ' first' : '') 
					. ($span > 1 ? ' span_'.esc_attr($span) : '') 
					. (yacht_rental_storage_get_array('sc_columns_data', 'after_span2') ? ' after_span_2' : '') 
					. (yacht_rental_storage_get_array('sc_columns_data', 'after_span3') ? ' after_span_3' : '') 
					. (yacht_rental_storage_get_array('sc_columns_data', 'after_span4') ? ' after_span_4' : '') 
					. '"'
					. ($css!='' ? ' style="'.esc_attr($css).'"' : '')
					. (!yacht_rental_param_is_off($animation) ? ' data-animation="'.esc_attr(yacht_rental_get_animation_classes($animation)).'"' : '')
					. '>'
					...
					. '</div>';
		yacht_rental_storage_inc_array('sc_columns_data', 'counter', $span);
		yacht_rental_storage_set_array('sc_columns_data', 'after_span2', $span==2);
		yacht_rental_storage_set_array('sc_columns_data', 'after_span3', $span==3);
		yacht_rental_storage_set_array('sc_columns_data', 'after_span4', $span==4);
		yacht_rental_storage_set_array('sc_columns_data', 'equal_selector', $bg_color!=='' || $bg_image !== '' ? '.sc_column_item_inner' : '.sc_column_item');
		return apply_filters('yacht_rental_shortcode_output', $output, 'trx_column_item', $atts, $content);
	}
	add_shortcode('trx_column_item', 'yacht_rental_sc_column_item');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'yacht_rental_sc_columns_reg_shortcodes' ) ) {
	function yacht_rental_sc_columns_reg_shortcodes() {
	
		yacht_rental_sc_map("trx_columns", array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'yacht_rental_sc_columns_reg_shortcodes_vc' ) ) {
	function yacht_rental_sc_columns_reg_shortcodes_vc() {
		...
		class WPBakeryShortCode_Trx_Columns extends Yacht_Rental_VC_ShortCodeColumns {}
		class WPBakeryShortCode_Trx_Column_Item extends Yacht_Rental_VC_ShortCodeCollection {}
	}
}
?>