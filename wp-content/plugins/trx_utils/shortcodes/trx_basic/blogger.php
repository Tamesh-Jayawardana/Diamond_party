...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('yacht_rental_sc_blogger_theme_setup')) {
	add_action( 'yacht_rental_action_before_init_theme', 'yacht_rental_sc_blogger_theme_setup' );
	function yacht_rental_sc_blogger_theme_setup() {
		add_action('yacht_rental_action_shortcodes_list', 		'yacht_rental_sc_blogger_reg_shortcodes');
		if (function_exists('yacht_rental_exists_visual_composer') && yacht_rental_exists_visual_composer())
			add_action('yacht_rental_action_shortcodes_list_vc','yacht_rental_sc_blogger_reg_shortcodes_vc');
	}
}
...
yacht_rental_storage_set('sc_blogger_busy', false);

if (!function_exists('yacht_rental_sc_blogger')) {	
	function yacht_rental_sc_blogger($atts, $content=null){	
		if (yacht_rental_in_shortcode_blogger(true)) return '';
		extract(yacht_rental_html_decode(shortcode_atts(array(
			...
			"location" => "default",
			"dir" => "horizontal",
			"hover" => yacht_rental_get_theme_option('hover_style'),
			"hover_dir" => yacht_rental_get_theme_option('hover_dir'),
			"scroll" => "no",
			"controls" => "no",
			"rating" => "no",
			"info" => "yes",
		...
		yacht_rental_storage_set('sc_blogger_busy', true);
		yacht_rental_storage_set('sc_blogger_counter', 0);
		...
		if ($descr == '') $descr = yacht_rental_get_custom_option('post_excerpt_maxlength'.($columns > 1 ? '_masonry' : ''));
	
		if (!yacht_rental_param_is_off($scroll)) {
			yacht_rental_enqueue_slider();
			if (empty($id)) $id = 'sc_blogger_'.str_replace('.', '', mt_rand());
		}
		
		$class = apply_filters('yacht_rental_filter_blog_class',
					'sc_blogger'
					. ' layout_'.esc_attr($style)
					. ' template_'.esc_attr(yacht_rental_get_template_name($style))
					. (!empty($class) ? ' '.esc_attr($class) : '')
					. ' ' . esc_attr(yacht_rental_get_template_property($style, 'container_classes'))
					. ' sc_blogger_' . ($dir=='vertical' ? 'vertical' : 'horizontal')
					. (yacht_rental_param_is_on($scroll) && yacht_rental_param_is_on($controls) ? ' sc_scroll_controls sc_scroll_controls_type_top sc_scroll_controls_'.esc_attr($dir) : '')
					. ($descr == 0 ? ' no_description' : ''),
					array('style'=>$style, 'dir'=>$dir, 'descr'=>$descr)
		);
	
		$container = apply_filters('yacht_rental_filter_blog_container', yacht_rental_get_template_property($style, 'container'), array('style'=>$style, 'dir'=>$dir));
		...
		$container2 = apply_filters('yacht_rental_filter_blog_container2', yacht_rental_get_template_property($style, 'container2'), array('style'=>$style, 'dir'=>$dir));
		...
		$output = '<div'
			...
			. '>'
			. ($container_start)
			. (!empty($subtitle) ? '<h6 class="sc_blogger_subtitle sc_item_subtitle">' . trim(yacht_rental_strmacros($subtitle)) . '</h6>' : '')
			. (!empty($title) ? '<h2 class="sc_blogger_title sc_item_title' . (empty($description) ? ' sc_item_title_without_descr' : ' sc_item_title_without_descr') . '">' . trim(yacht_rental_strmacros($title)) . '</h2>' : '')
			. (!empty($description) ? '<div class="sc_blogger_descr sc_item_descr">' . trim(yacht_rental_strmacros($description)) . '</div>' : '')
			. ($container2_start)
			. ($style=='list' ? '<ul class="sc_list sc_list_style_iconed">' : '')
			. ($dir=='horizontal' && $columns > 1 && yacht_rental_get_template_property($style, 'need_columns') ? '<div class="columns_wrap">' : '')
		...
		if (yacht_rental_get_template_property($style, 'need_isotope')) {
			if (!yacht_rental_param_is_off($filters))
				$output .= '<div class="isotope_filters"></div>';
			if ($columns<1) $columns = yacht_rental_substr($style, -1);
		...
		if (!yacht_rental_param_is_off($only)) $args = yacht_rental_query_add_filters($args, $only);
		...
		while ( $query->have_posts() ) { $query->the_post();
			...
			yacht_rental_storage_inc('sc_blogger_counter');
	
			$args = array(
				'layout' => $style,
				'show' => false,
				'number' => yacht_rental_storage_get('sc_blogger_counter'),
				'add_view_more' => false,
				...
				"readmore" => $readmore,
				"loadmore" => $loadmore,
				"reviews" => yacht_rental_param_is_on($rating),
				"dir" => $dir,
				"scroll" => yacht_rental_param_is_on($scroll),
				"info" => yacht_rental_param_is_on($info),
				...
				// Get post data
				'strip_teaser' => false,
				'content' => yacht_rental_get_template_property($style, 'need_content'),
				'terms_list' => !yacht_rental_param_is_off($filters) || yacht_rental_get_template_property($style, 'need_terms'),
				'filters' => yacht_rental_param_is_off($filters) ? '' : $filters,
				'hover' => $hover,
				'hover_dir' => $hover_dir
			);
		...
		if (!yacht_rental_param_is_off($filters)) {
			...
			if ($filters == 'categories') {			// Use categories as filter items
				...
				$portfolio_parent = $cat ? max(0, yacht_rental_get_parent_taxonomy_by_property($cat, 'show_filters', 'yes', true, $taxonomy)) : 0;
				$args2 = array(
					'type'			=> $post_type,
		...
		$output	.= (yacht_rental_param_is_on($scroll) 
				? '</div></div><div id="'.esc_attr($id).'_scroll_bar" class="sc_scroll_bar sc_scroll_bar_'.esc_attr($dir).' '.esc_attr($id).'_scroll_bar"></div></div>'
					. (!yacht_rental_param_is_off($controls) ? '<div class="sc_scroll_controls_wrap"><a class="sc_scroll_prev" href="#"></a><a class="sc_scroll_next" href="#"></a></div>' : '')
				: '')
			. ($dir=='horizontal' && $columns > 1 && yacht_rental_get_template_property($style, 'need_columns') ? '</div>' :  '')
			. ($style == 'list' ? '</ul>' : '')
			. ($container2_end)
			. (!empty($link) 
		...
		return apply_filters('yacht_rental_shortcode_output', $output, 'trx_blogger', $atts, $content);
	}
	add_shortcode('trx_blogger', 'yacht_rental_sc_blogger');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'yacht_rental_sc_blogger_reg_shortcodes' ) ) {
	function yacht_rental_sc_blogger_reg_shortcodes() {
	
		yacht_rental_sc_map("trx_blogger", array(
			...
			"params" => array(
				...
				"style" => array(
					...
					"value" => "accordion",
					"type" => "select",
					"options" => yacht_rental_get_sc_param('blogger_styles')
				),
//				"filters" => array(
...
//					"dir" => "horizontal",
//					"type" => "checklist",
//					"options" => yacht_rental_get_sc_param('filters')
//				),
				"hover" => array(
					...
					"value" => "",
					"type" => "select",
					"options" => yacht_rental_get_sc_param('hovers')
				),
				"hover_dir" => array(
					...
					"value" => "left_to_right",
					"type" => "select",
					"options" => yacht_rental_get_sc_param('hovers_dir')
				),
				"dir" => array(
				...
				"post_type" => array(
					...
					"value" => "post",
					"type" => "select",
					"options" => yacht_rental_get_sc_param('posts_types')
				),
				"ids" => array(
				...
				"only" => array(
					...
					"value" => "no",
					"type" => "select",
					"options" => yacht_rental_get_sc_param('formats')
				),
				"scroll" => array(
				...
				"location" => array(
					...
					"value" => "default",
					"type" => "select",
					"options" => yacht_rental_get_sc_param('locations')
				),
				"rating" => array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'yacht_rental_sc_blogger_reg_shortcodes_vc' ) ) {
	function yacht_rental_sc_blogger_reg_shortcodes_vc() {
		...
		vc_map( array(
			...
			"params" => array(
				array(
					...
					"std" => "accordion",
					"class" => "",
					"value" => array_flip(yacht_rental_get_sc_param('blogger_styles')),
					"type" => "dropdown"
				),
//				array(
//					"param_name" => "filters",
...
//					"admin_label" => true,
//					"class" => "",
//					"value" => array_flip(yacht_rental_get_sc_param('filters')),
//					"type" => "dropdown"
//				),
				array(
					"param_name" => "hover",
					...
					"class" => "",
					"value" => array_flip((array)yacht_rental_get_sc_param('hovers')),
					'dependency' => array(
						'element' => 'style',
				...
				array(
					...
					"class" => "",
					"value" => array_flip((array)yacht_rental_get_sc_param('hovers_dir')),
					'dependency' => array(
						'element' => 'style',
						'value' => array('portfolio_2','portfolio_3','portfolio_4','grid_2','grid_3','grid_4','square_2','square_3','square_4','short_2','short_3','short_4','colored_2','colored_3','colored_4')
					),
				...
				array(
					...
					"value" => array_flip((array)yacht_rental_get_sc_param('locations')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "dir",
				...
				array(
					...
					"class" => "",
					"value" => array_flip((array)yacht_rental_get_sc_param('posts_types')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "ids",
				...
				array(
					...
					"class" => "",
					"group" => esc_html__('Query', 'trx_utils'),
					"value" => array_flip((array)yacht_rental_get_sc_param('formats')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "scroll",
		...
		class WPBakeryShortCode_Trx_Blogger extends Yacht_Rental_VC_ShortCodeSingle {}
	}
}
?>