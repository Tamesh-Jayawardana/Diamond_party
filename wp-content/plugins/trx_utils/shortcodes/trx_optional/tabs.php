...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('yacht_rental_sc_tabs_theme_setup')) {
	add_action( 'yacht_rental_action_before_init_theme', 'yacht_rental_sc_tabs_theme_setup' );
	function yacht_rental_sc_tabs_theme_setup() {
		add_action('yacht_rental_action_shortcodes_list', 		'yacht_rental_sc_tabs_reg_shortcodes');
		if (function_exists('yacht_rental_exists_visual_composer') && yacht_rental_exists_visual_composer())
			add_action('yacht_rental_action_shortcodes_list_vc','yacht_rental_sc_tabs_reg_shortcodes_vc');
	}
}
...
if (!function_exists('yacht_rental_sc_tabs')) {	
	function yacht_rental_sc_tabs($atts, $content = null) {
		...
		yacht_rental_storage_set('sc_tab_data', array(
			'counter'=> 0,
            'scroll' => $scroll,
		...
		$sc_tab_titles = yacht_rental_storage_get_array('sc_tab_data', 'titles');
		...
		$tabs_output .= $titles_output
			...
			. ($content)
			.'</div>';
		return apply_filters('yacht_rental_shortcode_output', $tabs_output, 'trx_tabs', $atts, $content);
	}
	add_shortcode("trx_tabs", "yacht_rental_sc_tabs");
}


if (!function_exists('yacht_rental_sc_tab')) {	
	function yacht_rental_sc_tab($atts, $content = null) {
		if (yacht_rental_in_shortcode_blogger()) return '';
		extract(yacht_rental_html_decode(shortcode_atts(array(
			// Individual params
		...
		yacht_rental_storage_inc_array('sc_tab_data', 'counter');
		$counter = yacht_rental_storage_get_array('sc_tab_data', 'counter');
		if (empty($id))
			$id = !empty($tab_id) ? $tab_id : yacht_rental_storage_get_array('sc_tab_data', 'id').'_'.intval($counter);
		$sc_tab_titles = yacht_rental_storage_get_array('sc_tab_data', 'titles');
		...
		yacht_rental_storage_set_array('sc_tab_data', 'titles', $sc_tab_titles);
		$output = '<div id="'.esc_attr($id).'"'
					.' class="sc_tabs_content' 
						...
						. ($css!='' ? ' style="'.esc_attr($css).'"' : '') 
						. '>' 
				. (yacht_rental_param_is_on(yacht_rental_storage_get_array('sc_tab_data', 'scroll')) 
					? '<div id="'.esc_attr($id).'_scroll" class="sc_scroll sc_scroll_vertical" style="height:'.(yacht_rental_storage_get_array('sc_tab_data', 'height') != '' ? yacht_rental_storage_get_array('sc_tab_data', 'height') : '200px').';"><div class="sc_scroll_wrapper swiper-wrapper"><div class="sc_scroll_slide swiper-slide">' 
					: '')
				...
				. (yacht_rental_param_is_on(yacht_rental_storage_get_array('sc_tab_data', 'scroll')) 
					...
					: '')
			. '</div>';
		return apply_filters('yacht_rental_shortcode_output', $output, 'trx_tab', $atts, $content);
	}
	add_shortcode("trx_tab", "yacht_rental_sc_tab");
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'yacht_rental_sc_tabs_reg_shortcodes' ) ) {
	function yacht_rental_sc_tabs_reg_shortcodes() {
	
		yacht_rental_sc_map("trx_tabs", array(
...
//					"options" => yacht_rental_get_list_styles(1, 2),
					"options" => yacht_rental_get_list_styles(1),
					"type" => "radio"
				),
				"initial" => array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'yacht_rental_sc_tabs_reg_shortcodes_vc' ) ) {
	function yacht_rental_sc_tabs_reg_shortcodes_vc() {
		...
		vc_map( array(
			...
			"params" => array(
				array(
					...
					"admin_label" => true,
					"class" => "",
//					"value" => array_flip(yacht_rental_get_list_styles(1, 2)),
					"value" => array_flip(yacht_rental_get_list_styles(1)),
					"type" => "dropdown"
				),
				array(
					"param_name" => "initial",
		...
		class WPBakeryShortCode_Trx_Tabs extends Yacht_Rental_VC_ShortCodeTabs {}
		class WPBakeryShortCode_Trx_Tab extends Yacht_Rental_VC_ShortCodeTab {}
	}
}
?>