...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('yacht_rental_sc_highlight_theme_setup')) {
	add_action( 'yacht_rental_action_before_init_theme', 'yacht_rental_sc_highlight_theme_setup' );
	function yacht_rental_sc_highlight_theme_setup() {
		add_action('yacht_rental_action_shortcodes_list', 		'yacht_rental_sc_highlight_reg_shortcodes');
		if (function_exists('yacht_rental_exists_visual_composer') && yacht_rental_exists_visual_composer())
			add_action('yacht_rental_action_shortcodes_list_vc','yacht_rental_sc_highlight_reg_shortcodes_vc');
	}
}
...
if (!function_exists('yacht_rental_sc_highlight')) {	
	function yacht_rental_sc_highlight($atts, $content=null){	
		...
		$css .= ($color != '' ? 'color:' . esc_attr($color) . ';' : '')
			...
			.($font_size != '' ? 'font-size:' . esc_attr(yacht_rental_prepare_css_value($font_size)) . '; line-height: 1em;' : '');
		$output = '<span' . ($id ? ' id="'.esc_attr($id).'"' : '') 
				...
				. '</span>';
		return apply_filters('yacht_rental_shortcode_output', $output, 'trx_highlight', $atts, $content);
	}
	add_shortcode('trx_highlight', 'yacht_rental_sc_highlight');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'yacht_rental_sc_highlight_reg_shortcodes' ) ) {
	function yacht_rental_sc_highlight_reg_shortcodes() {
	
		yacht_rental_sc_map("trx_highlight", array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'yacht_rental_sc_highlight_reg_shortcodes_vc' ) ) {
	function yacht_rental_sc_highlight_reg_shortcodes_vc() {
		...
		class WPBakeryShortCode_Trx_Highlight extends Yacht_Rental_VC_ShortCodeSingle {}
	}
}
?>