...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('yacht_rental_sc_icon_theme_setup')) {
	add_action( 'yacht_rental_action_before_init_theme', 'yacht_rental_sc_icon_theme_setup' );
	function yacht_rental_sc_icon_theme_setup() {
		add_action('yacht_rental_action_shortcodes_list', 		'yacht_rental_sc_icon_reg_shortcodes');
		if (function_exists('yacht_rental_exists_visual_composer') && yacht_rental_exists_visual_composer())
			add_action('yacht_rental_action_shortcodes_list_vc','yacht_rental_sc_icon_reg_shortcodes_vc');
	}
}
...
if (!function_exists('yacht_rental_sc_icon')) {	
	function yacht_rental_sc_icon($atts, $content=null){	
		...
		$css2 = ($font_weight != '' && !yacht_rental_is_inherit_option($font_weight) ? 'font-weight:'. esc_attr($font_weight).';' : '')
			. ($font_size != '' ? 'font-size:' . esc_attr(yacht_rental_prepare_css_value($font_size)) . '; line-height: ' . (!$bg_shape || yacht_rental_param_is_inherit($bg_shape) ? '1' : '1.2') . 'em;' : '')
		...
		$output = $icon!='' 
			? ($link ? '<a href="'.esc_url($link).'"' : '<span') . ($id ? ' id="'.esc_attr($id).'"' : '')
				. ' class="sc_icon '.esc_attr($icon)
					. ($bg_shape && !yacht_rental_param_is_inherit($bg_shape) ? ' sc_icon_shape_'.esc_attr($bg_shape) : '')
				...
				.($link ? '</a>' : '</span>')
			: '';
		return apply_filters('yacht_rental_shortcode_output', $output, 'trx_icon', $atts, $content);
	}
	add_shortcode('trx_icon', 'yacht_rental_sc_icon');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'yacht_rental_sc_icon_reg_shortcodes' ) ) {
	function yacht_rental_sc_icon_reg_shortcodes() {
	
		yacht_rental_sc_map("trx_icon", array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'yacht_rental_sc_icon_reg_shortcodes_vc' ) ) {
	function yacht_rental_sc_icon_reg_shortcodes_vc() {
		...
		class WPBakeryShortCode_Trx_Icon extends Yacht_Rental_VC_ShortCodeSingle {}
	}
}
?>