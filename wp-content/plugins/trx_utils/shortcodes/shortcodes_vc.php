...
if ( !function_exists( 'yacht_rental_vc_width' ) ) {
	function yacht_rental_vc_width($w='') {
		return array(
			"param_name" => "width",
...
if ( !function_exists( 'yacht_rental_vc_height' ) ) {
	function yacht_rental_vc_height($h='') {
		return array(
			"param_name" => "height",
...
if ( !function_exists( 'yacht_rental_shortcodes_vc_scripts_admin' ) ) {
	function yacht_rental_shortcodes_vc_scripts_admin() {
		// Include CSS 
...
// Load scripts and styles for VC support
if ( !function_exists( 'yacht_rental_shortcodes_vc_scripts_front' ) ) {
	function yacht_rental_shortcodes_vc_scripts_front() {
...
if ( !function_exists( 'yacht_rental_shortcodes_vc_add_init_script' ) ) {
	function yacht_rental_shortcodes_vc_add_init_script($output, $tag='', $atts=array(), $content='') {
		if ( (isset($_GET['vc_editable']) && $_GET['vc_editable']=='true') && (isset($_POST['action']) && $_POST['action']=='vc_load_shortcode')
				&& ( isset($_POST['shortcodes'][0]['tag']) && $_POST['shortcodes'][0]['tag']==$tag )
		) {
			if (yacht_rental_strpos($output, 'yacht_rental_vc_init_shortcodes')===false) {
				$id = "yacht_rental_vc_init_shortcodes_".str_replace('.', '', mt_rand());
				...
				$output .= '<'.trim($holder).' id="'.esc_attr($id).'">
						try {
							...
							yacht_rental_init_shortcodes(jQuery("body").eq(0));
...
if ( !function_exists( 'yacht_rental_get_vc_param' ) ) {
	function yacht_rental_get_vc_param($prm) {
		return yacht_rental_storage_get_array('vc_params', $prm);
	}
}

// Set vc_param value
if ( !function_exists( 'yacht_rental_set_vc_param' ) ) {
	function yacht_rental_set_vc_param($prm, $val) {
		yacht_rental_storage_set_array('vc_params', $prm, $val);
	}
}
...
if ( !function_exists( 'yacht_rental_shortcodes_vc_theme_setup' ) ) {
	//if ( yacht_rental_vc_is_frontend() )
	if ( (isset($_GET['vc_editable']) && $_GET['vc_editable']=='true') || (isset($_GET['vc_action']) && $_GET['vc_action']=='vc_inline') )
		add_action( 'yacht_rental_action_before_init_theme', 'yacht_rental_shortcodes_vc_theme_setup', 20 );
	else
		add_action( 'yacht_rental_action_after_init_theme', 'yacht_rental_shortcodes_vc_theme_setup' );
	function yacht_rental_shortcodes_vc_theme_setup() {
		...
		vc_add_param("vc_row", array(
					...
					"class" => "",
					"value" => array_flip(yacht_rental_get_list_color_schemes(true)),
					"type" => "dropdown"
		));
		vc_add_param("vc_row", array(
					"param_name" => "inverse",
		...
		if (yacht_rental_shortcodes_is_used() && class_exists('Yacht_Rental_VC_ShortCodeSingle')) {
			...
			// Load scripts and styles for VC support
			add_action( 'wp_enqueue_scripts',		'yacht_rental_shortcodes_vc_scripts_front');
			add_action( 'admin_enqueue_scripts',	'yacht_rental_shortcodes_vc_scripts_admin' );

			// Add init script into shortcodes output in VC frontend editor
			add_filter('yacht_rental_shortcode_output', 'yacht_rental_shortcodes_vc_add_init_script', 10, 4);

			yacht_rental_storage_set('vc_params', array(
				...
				'animation' => array(
					...
					"group" => esc_html__('ID &amp; Class', 'trx_utils'),
					"class" => "",
					"value" => array_flip((array)yacht_rental_get_sc_param('animations')),
					"type" => "dropdown"
				),
				...
				'margin_top' => array(
					...
					"group" => esc_html__('Size &amp; Margins', 'trx_utils'),
					"std" => "inherit",
					"value" => array_flip((array)yacht_rental_get_sc_param('margins')),
					"type" => "dropdown"
				),
			...
			do_action('yacht_rental_action_shortcodes_list_vc');
