...
// Theme init
if (!function_exists('yacht_rental_widget_recent_news_theme_setup')) {
	add_action( 'yacht_rental_action_before_init_theme', 'yacht_rental_widget_recent_news_theme_setup', 1 );
	function yacht_rental_widget_recent_news_theme_setup() {

		// Register shortcodes in the shortcodes list
		if (function_exists('yacht_rental_exists_visual_composer') && yacht_rental_exists_visual_composer())
			add_action('yacht_rental_action_shortcodes_list_vc','yacht_rental_widget_recent_news_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('yacht_rental_widget_recent_news_load')) {
	add_action( 'widgets_init', 'yacht_rental_widget_recent_news_load' );
	function yacht_rental_widget_recent_news_load() {
		register_widget('yacht_rental_widget_recent_news');
	}
}


// Widget Class
class yacht_rental_widget_recent_news extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_recent_news', 'description' => esc_html__('Show recent news in many styles', 'trx_utils'));
		parent::__construct( 'yacht_rental_widget_recent_news', esc_html__('Yacht rental - Recent News', 'trx_utils'), $widget_ops );
	}
	...
	function widget($args, $instance) {
		...
		$output = yacht_rental_sc_recent_news( array(
			'from_widget'		=> true,
		...
		if (!empty($output)) {
			...
			if ($widget_title) yacht_rental_show_layout($title, $before_title, $after_title);
	...
	function form($instance) {
		...
		$posts_types = yacht_rental_get_list_posts_types(false);
		$categories = yacht_rental_get_list_terms(false, yacht_rental_get_taxonomy_categories_by_post_type($post_type));
		$news_styles = yacht_rental_get_list_templates('news');
		?>
...
*/
if ( !function_exists( 'yacht_rental_sc_recent_news' ) ) {
	function yacht_rental_sc_recent_news($atts, $content=null){	
		...
		// Get categories list
		if (yacht_rental_param_is_on($show_categories)) {
			if ( !yacht_rental_storage_isset('categories_'.$category) ) {
				yacht_rental_storage_set('categories_'.$category, get_categories(array(
					'orderby' => 'name',
					'parent' => $category
					))
				);
		...
		if (empty($from_widget)) {
			$widget_args = yacht_rental_prepare_widgets_args(yacht_rental_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_recent_news', 'widget_recent_news');
			$output .= '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_recent_news_wrap' 
								. (yacht_rental_exists_visual_composer() ? ' vc_recent_news wpb_content_element' : '') 
		...
		// Header
		if ( !empty($title) || !empty($subtitle) || (yacht_rental_param_is_on($show_categories) && !yacht_rental_storage_empty('categories_'.$category)) ) {
			$output	.= '<div class="sc_recent_news_header'.(yacht_rental_param_is_on($show_categories) && !yacht_rental_storage_empty('categories_'.$category) ? ' sc_recent_news_header_split' : '').'">'
							. ( !empty($title) || !empty($subtitle)
								? '<div class="sc_recent_news_header_captions">'
										. (!empty($title) ? '<h1 class="sc_recent_news_title">' . esc_html($title) . '</h1>' : '')
									. '</div>'
								: '');

			// Categories list
			$cat_list = yacht_rental_storage_get('categories_'.$category);
			if (yacht_rental_param_is_on($show_categories) && !empty($cat_list)) {
				...
				if (is_array($cat_list) && count($cat_list) > 0) {
					$output .= '<a href="' . esc_url( $category == 0 
						...
						: get_category_link($category) ) . '" class="sc_recent_news_header_category_item">'.esc_html__('All News', 'trx_utils').'</a>';
					$number = 0;
					$number_max = 3;
		...
		$args = yacht_rental_query_add_posts_and_cats($args, $ids, $post_type, $category, $taxonomy);
		...
		return apply_filters('yacht_rental_shortcode_output', $output, 'trx_recent_news', $atts, $content);
	}
	add_shortcode("trx_recent_news", "yacht_rental_sc_recent_news");
}


// Add [trx_recent_news] in the VC shortcodes list
if (!function_exists('yacht_rental_widget_recent_news_reg_shortcodes_vc')) {
	function yacht_rental_widget_recent_news_reg_shortcodes_vc() {
		
		$posts_types = yacht_rental_get_list_posts_types(false);
		$categories = yacht_rental_get_list_terms(false, yacht_rental_get_taxonomy_categories_by_post_type('post'));
		$news_styles = yacht_rental_get_list_templates('news');
		...
		vc_map( array(
				...
				"params" => array(
					...
					array(
						...
						"group" => esc_html__('Query', 'trx_utils'),
						"class" => "",
						"value" => array_flip((array)yacht_rental_array_merge(array(0 => esc_html__('- Select category -', 'trx_utils')), $categories)),
						"type" => "dropdown"
					),
					array(
						"param_name" => "count",
					...
					array(
						...
						"group" => esc_html__('Query', 'trx_utils'),
						"class" => "",
						"value" => array_flip(yacht_rental_get_list_sortings()),
						"type" => "dropdown"
					),
					array(
						"param_name" => "order",
						...
						"group" => esc_html__('Query', 'trx_utils'),
						"class" => "",
						"value" => array_flip((array)yacht_rental_get_list_orderings()),
						"type" => "dropdown"
					),
					array(
						"param_name" => "ids",
