...
jQuery(document).ready(function(){
	yacht_rental_qrcode_init();
	yacht_rental_color_picker();
});

function yacht_rental_qrcode_init() {
	jQuery('#widgets-right .widget_qrcode:not(.inited)').each(function() {
		...
		widget.find('input.iColorPicker:not(.colored)').each(function() {
			...
			if (obj.attr('id').indexOf('__i__') < 0) {
				...
				yacht_rental_set_color_picker(obj.attr('id'));
			}
		});
		widget.find('div.qrcode_tabs').tabs();
		widget.on('click', '.fld_button_draw', function() {
			yacht_rental_qrcode_update(widget);
		});
...
function yacht_rental_qrcode_update(widget) {
	yacht_rental_qrcode_show(widget, {
...
function yacht_rental_qrcode_show(widget, vc, opt) {
	if (opt.show_what==1) {
...
if (!window.yacht_rental_set_color_picker) {
	function yacht_rental_set_color_picker(id_picker) {
