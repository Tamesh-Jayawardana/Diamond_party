...
// Theme init
if (!function_exists('yacht_rental_widget_calendar_theme_setup')) {
	add_action( 'yacht_rental_action_before_init_theme', 'yacht_rental_widget_calendar_theme_setup', 1 );
	function yacht_rental_widget_calendar_theme_setup() {

		// Register shortcodes in the shortcodes list
		if (function_exists('yacht_rental_exists_visual_composer') && yacht_rental_exists_visual_composer())
			add_action('yacht_rental_action_shortcodes_list_vc','yacht_rental_widget_calendar_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('yacht_rental_widget_calendar_load')) {
	add_action( 'widgets_init', 'yacht_rental_widget_calendar_load' );
	function yacht_rental_widget_calendar_load() {
		register_widget('yacht_rental_widget_calendar');
	}
}

// Widget Class
class yacht_rental_widget_calendar extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_calendar', 'description' => esc_html__('Calendar for posts and/or Events', 'trx_utils'));
		parent::__construct( 'yacht_rental_widget_calendar', esc_html__('Yacht rental - Advanced Calendar', 'trx_utils'), $widget_ops );
	}
	...
	function widget($args, $instance) {
		...
		$output = yacht_rental_get_calendar(false, 0, 0, array('post_type'=>$post_type));
...
*/
if ( !function_exists( 'yacht_rental_sc_widget_calendar' ) ) {
	function yacht_rental_sc_widget_calendar($atts, $content=null){	
		...
		extract($atts);
		$type = 'yacht_rental_widget_calendar';
		$output = '';
		global $wp_widget_factory;
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_widget_calendar' 
								. (yacht_rental_exists_visual_composer() ? ' vc_widget_calendar wpb_content_element' : '') 
								. (!empty($class) ? ' ' . esc_attr($class) : '') 
						. '">';
			ob_start();
			the_widget( $type, $atts, yacht_rental_prepare_widgets_args(yacht_rental_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_calendar', 'widget_calendar') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('yacht_rental_shortcode_output', $output, 'trx_widget_calendar', $atts, $content);
	}
	add_shortcode("trx_widget_calendar", "yacht_rental_sc_widget_calendar");
}


// Add [trx_widget_calendar] in the VC shortcodes list
if (!function_exists('yacht_rental_widget_calendar_reg_shortcodes_vc')) {
	function yacht_rental_widget_calendar_reg_shortcodes_vc() {
