...
// Theme init
if (!function_exists('yacht_rental_widget_banner_theme_setup')) {
	add_action( 'yacht_rental_action_before_init_theme', 'yacht_rental_widget_banner_theme_setup', 1 );
	function yacht_rental_widget_banner_theme_setup() {

		// Register shortcodes in the shortcodes list
		if (function_exists('yacht_rental_exists_visual_composer') && yacht_rental_exists_visual_composer())
			add_action('yacht_rental_action_shortcodes_list_vc','yacht_rental_widget_banner_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('yacht_rental_widget_banner_load')) {
	add_action( 'widgets_init', 'yacht_rental_widget_banner_load' );
	function yacht_rental_widget_banner_load() {
		register_widget( 'yacht_rental_widget_banner' );
	}
}

// Widget Class
class yacht_rental_widget_banner extends WP_Widget {

	function __construct() {
		$widget_ops = array( 'classname' => 'widget_banner', 'description' => esc_html__('Banner', 'trx_utils') );
		parent::__construct( 'yacht_rental_widget_banner', esc_html__('Yacht rental - Banner', 'trx_utils'), $widget_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		<div class="widget_banner_inner">
			...
			if ($banner_image!='') {
				...
				$attr = yacht_rental_getimagesize($banner_image);
			...
			if ($banner_code!='') {
				echo force_balance_tags(yacht_rental_substitute_all($banner_code));
			}
			?>
		</div>
		<?php
	...
	function form( $instance ) {
		...
		<p>
            ...
            <?php
			yacht_rental_show_layout(yacht_rental_show_custom_field($this->get_field_id( 'banner_media' ), array('type'=>'mediamanager', 'media_field_id'=>$this->get_field_id( 'banner_image' )), null));
			if ($banner_image) {
			?>
...
*/
if ( !function_exists( 'yacht_rental_sc_widget_banner' ) ) {
	function yacht_rental_sc_widget_banner($atts, $content=null){	
		...
		extract($atts);
		$type = 'yacht_rental_widget_banner';
		$output = '';
		...
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			...
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_widget_banner' 
								. (yacht_rental_exists_visual_composer() ? ' vc_widget_banner wpb_content_element' : '') 
								. (!empty($class) ? ' ' . esc_attr($class) : '') 
						. '">';
			ob_start();
			the_widget( $type, $atts, yacht_rental_prepare_widgets_args(yacht_rental_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_banner', 'widget_banner') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('yacht_rental_shortcode_output', $output, 'trx_widget_banner', $atts, $content);
	}
	add_shortcode("trx_widget_banner", "yacht_rental_sc_widget_banner");
}


// Add [trx_widget_banner] in the VC shortcodes list
if (!function_exists('yacht_rental_widget_banner_reg_shortcodes_vc')) {
	function yacht_rental_widget_banner_reg_shortcodes_vc() {
