...
// Theme init
if (!function_exists('yacht_rental_widget_recent_posts_theme_setup')) {
	add_action( 'yacht_rental_action_before_init_theme', 'yacht_rental_widget_recent_posts_theme_setup', 1 );
	function yacht_rental_widget_recent_posts_theme_setup() {

		// Register shortcodes in the shortcodes list
		if (function_exists('yacht_rental_exists_visual_composer') && yacht_rental_exists_visual_composer())
			add_action('yacht_rental_action_shortcodes_list_vc','yacht_rental_widget_recent_posts_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('yacht_rental_widget_recent_posts_load')) {
	add_action( 'widgets_init', 'yacht_rental_widget_recent_posts_load' );
	function yacht_rental_widget_recent_posts_load() {
		register_widget('yacht_rental_widget_recent_posts');
	}
}

// Widget Class
class yacht_rental_widget_recent_posts extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_recent_posts', 'description' => esc_html__('The recent blog posts (extended)', 'trx_utils'));
		parent::__construct( 'yacht_rental_widget_recent_posts', esc_html__('Yacht rental - Recent Posts', 'trx_utils'), $widget_ops );
	...
	function widget($args, $instance) {
		...
		if (!empty($output)) {
			...
			if ($title) yacht_rental_show_layout($title, $before_title, $after_title);
	...
	function form($instance) {
		...
		$counters = array_merge(array('hide' => esc_html__('Hide', 'trx_utils')), yacht_rental_get_list_blog_counters(false));
		?>
...
*/
if ( !function_exists( 'yacht_rental_sc_widget_recent_posts' ) ) {
	function yacht_rental_sc_widget_recent_posts($atts, $content=null){	
		$atts = yacht_rental_html_decode(shortcode_atts(array(
			// Individual params
			"title" => "",
			"number" => 4,
			"show_date" => 1,
		...
		extract($atts);
		$type = 'yacht_rental_widget_recent_posts';
		$output = '';
		global $wp_widget_factory;
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_widget_recent_posts' 
								. (yacht_rental_exists_visual_composer() ? ' vc_widget_recent_posts wpb_content_element' : '') 
								. (!empty($class) ? ' ' . esc_attr($class) : '') 
						. '">';
			ob_start();
			the_widget( $type, $atts, yacht_rental_prepare_widgets_args(yacht_rental_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_recent_posts', 'widget_recent_posts') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('yacht_rental_shortcode_output', $output, 'trx_widget_recent_posts', $atts, $content);
	}
	add_shortcode("trx_widget_recent_posts", "yacht_rental_sc_widget_recent_posts");
}


// Add [trx_widget_recent_posts] in the VC shortcodes list
if (!function_exists('yacht_rental_widget_recent_posts_reg_shortcodes_vc')) {
	function yacht_rental_widget_recent_posts_reg_shortcodes_vc() {
