...
require_once 'includes/support.boats.php';
...
if (!function_exists('trx_utils_setup_widgets')) {
    ...
    function trx_utils_setup_widgets() {
        ...
        require_once trx_utils_get_file_dir('widgets/boats.php');
...
if (!function_exists('trx_utils_footer_add_scroll_to_top')) {
    add_action('wp_footer',								'trx_utils_footer_add_scroll_to_top', 1);
    function trx_utils_footer_add_scroll_to_top() {
...
if(!function_exists('trx_utils_filter_options')){
    add_filter('yacht_rental_filter_options', 'trx_utils_filter_options');
...
if ( !function_exists( 'trx_utils_core_frontend_scripts' ) ) {
    function trx_utils_core_frontend_scripts() {
        // Google map
        if ( function_exists('yacht_rental_get_theme_option') && yacht_rental_get_custom_option('show_googlemap')=='yes' && yacht_rental_get_theme_option('api_google') != '') {
            ...
            wp_enqueue_script( 'yacht-rental-googlemap-script', trx_utils_get_file_url('js/core.googlemap.js'), array(), null, true );
        }

        if ( is_single() && function_exists('yacht_rental_get_theme_option') && yacht_rental_get_custom_option('show_reviews')=='yes' ) {
            wp_enqueue_script( 'yacht-rental-core-reviews-script', trx_utils_get_file_url('js/core.reviews.js'), array('jquery'), null, true );
        }
    }
}
...
if (!function_exists('trx_utils_post_admin_scripts')) {
    function trx_utils_post_admin_scripts() {
        ...
        if (isset($BREWERY_GLOBALS['post_override_options']) && $BREWERY_GLOBALS['post_override_options']['page']=='post' && yacht_rental_storage_isset('options', 'show_reviews'))
            wp_enqueue_script( 'yacht-rental-core-reviews-script', trx_utils_get_file_url('js/core.reviews.js'), array('jquery'), null, true );
    }
}
