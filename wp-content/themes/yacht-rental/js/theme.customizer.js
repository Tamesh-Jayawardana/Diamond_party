...
function yacht_rental_theme_customizer(option, val) {
"use strict";
	var custom_style = '';
	...
	if (option == 'body_style') {
		...
		var params = YACHT_RENTAL_STORAGE['co_add_params']!=undefined ? YACHT_RENTAL_STORAGE['co_add_params'] : {};
		params[option] = val;
	...
	if (custom_style != '') {
		var styles = jQuery('#yacht_rental-customizer-styles-'+option).length > 0 ? jQuery('#yacht_rental-customizer-styles-'+option) : '';
		if (styles.length == 0)
			jQuery('head').append('<style id="yacht_rental-customizer-styles-'+option+'" type="text/css">'+custom_style+'</style>');
		else
