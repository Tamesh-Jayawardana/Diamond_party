...
if ( !function_exists( 'yacht_rental_template_boats_1_theme_setup' ) ) {
	add_action( 'yacht_rental_action_before_init_theme', 'yacht_rental_template_boats_1_theme_setup', 1 );
	function yacht_rental_template_boats_1_theme_setup() {
		yacht_rental_add_template(array(
			'layout' => 'boats-1',
			'template' => 'boats-1',
			'mode'   => 'boats,blog',
			'title'  => esc_html__('Boats /Style 1/', 'yacht-rental'),
...
// Template output
if ( !function_exists( 'yacht_rental_template_boats_1_output' ) ) {
	function yacht_rental_template_boats_1_output($post_options, $post_data) {
		$show_title = !empty($post_data['post_title']);
		$parts = explode('_', $post_options['layout']);
		$style = $parts[0];
		$columns = max(1, min(12, empty($parts[1]) ? (!empty($post_options['columns_count']) ? $post_options['columns_count'] : 2) : (int) $parts[1]));
		...
		?>
			<div<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?> class="sc_boats_item sc_boats_item_<?php echo esc_attr($post_options['number']) . ($post_options['number'] % 2 == 1 ? ' odd' : ' even') . ($post_options['number'] == 1 ? ' first' : '').(!empty($post_options['tag_class']) ? ' '.esc_attr($post_options['tag_class']) : ''); ?>"<?php echo (!empty($post_options['tag_css']) ? ' style="'.esc_attr($post_options['tag_css']).'"' : '') . (!yacht_rental_param_is_off($post_options['tag_animation']) ? ' data-animation="'.esc_attr(yacht_rental_get_animation_classes($post_options['tag_animation'])).'"' : '');?>>
				<?php if ($post_options['boat_image']) { ?>
				<div class="sc_boat_image"><?php
					echo (!empty($post_options['boat_link']) ? '<a href="'.esc_url($post_options['boat_link']).'">'	: '')
						. trim($post_options['boat_image'])
						. (!empty($post_options['boat_link']) ? '</a>' : ''); 
				?></div>
				<?php } ?>
			</div>
		<?php
