...
if ( !function_exists( 'yacht_rental_template_portfolio_theme_setup' ) ) {
	add_action( 'yacht_rental_action_before_init_theme', 'yacht_rental_template_portfolio_theme_setup', 1 );
	function yacht_rental_template_portfolio_theme_setup() {
		yacht_rental_add_template(array(
			'layout' => 'portfolio_2',
			'template' => 'portfolio',
			'mode'   => 'blog',
			'need_isotope' => true,
			'title'  => esc_html__('Portfolio tile (with hovers, different height) /2 columns/', 'yacht-rental'),
		...
		yacht_rental_add_template(array(
			...
			'mode'   => 'blog',
			'need_isotope' => true,
			'title'  => esc_html__('Portfolio tile /3 columns/', 'yacht-rental'),
			'thumb_title'  => esc_html__('Medium image', 'yacht-rental'),
			'w'		 => 370,
			'h_crop' => 209,
			'h'		 => null
		...
		yacht_rental_add_template(array(
			...
			'mode'   => 'blog',
			'need_isotope' => true,
			'title'  => esc_html__('Portfolio tile /4 columns/', 'yacht-rental'),
			'thumb_title'  => esc_html__('Medium image', 'yacht-rental'),
			'w'		 => 370,
			'h_crop' => 209,
			'h'		 => null
		...
		add_action('yacht_rental_action_blog_scripts', 'yacht_rental_template_portfolio_add_scripts');
	}
}

// Add template specific scripts
if (!function_exists('yacht_rental_template_portfolio_add_scripts')) {
	function yacht_rental_template_portfolio_add_scripts($style) {
		if (yacht_rental_substr($style, 0, 10) == 'portfolio_' 
			|| yacht_rental_substr($style, 0, 5) == 'grid_' 
			|| yacht_rental_substr($style, 0, 7) == 'square_' 
			|| yacht_rental_substr($style, 0, 6) == 'short_'
			|| yacht_rental_substr($style, 0, 6) == 'alter_' 
			|| yacht_rental_substr($style, 0, 8) == 'colored_') {
...
// Template output
if ( !function_exists( 'yacht_rental_template_portfolio_output' ) ) {
	function yacht_rental_template_portfolio_output($post_options, $post_data) {
		...
		if ($style == 'colored' && $columns==1) {				// colored excerpt style (1 column)
			...
			<div class="isotope_item isotope_item_colored isotope_item_colored_1 isotope_column_1
						...
						?>">
				<<?php yacht_rental_show_layout($tag); ?> class="post_item post_item_colored post_item_colored_1
					...
					<div class="post_content isotope_item_content">
						<div class="post_featured img">
							...
							yacht_rental_show_layout($post_data['post_thumb'], $link_start, $link_end);
							...
							$new = yacht_rental_get_custom_option('mark_as_new', '', $post_data['post_id'], $post_data['post_type']);					// !!!!!! Get option from specified post
							if ($new && $new > date('Y-m-d')) {
								?><div class="post_mark_new"><?php esc_html_e('NEW', 'yacht-rental'); ?></div><?php
							}
							?>
						</div>
						...
						<div class="post_description clearfix">
							...
							if ($post_data['post_type'] == 'matches'){
								...
								echo '<div class="match_date">'
										. (!empty($post_meta['match_date']) && !yacht_rental_param_is_inherit($post_meta['match_date']) ?  date("d M", strtotime($post_meta['match_date'])) : '')
										. (!empty($post_meta['match_time']) && !yacht_rental_param_is_inherit($post_meta['match_time']) ?  ' ' . trim($post_meta['match_time']) : '')
									.'</div>';
								?>
							...
							?>
							<h5 class="post_title"><?php yacht_rental_show_layout($post_data['post_title'] . ($post_data['post_type'] == 'matches' ? ': ' . trim($post_meta['match_player_1']) . ' - ' . trim($post_meta['match_player_2']) : ''), $link_start, $link_end); ?></h5>
							<?php
							if ($post_data['post_type'] == 'players'){
								// Player information
								echo '<div class="player_info">'
										. (!empty($post_meta['player_country']) && !yacht_rental_param_is_inherit($post_meta['player_country']) ? '<span class="player_country">'. esc_html__( 'Country: ', 'yacht-rental' ) .''. yacht_rental_get_list_countries(false,$post_meta['player_country']) . '</span>' : '')
										. (!empty($post_meta['player_club']) && !yacht_rental_param_is_inherit($post_meta['player_club']) ? '<span class="player_club">'. esc_html__( 'Club: ', 'yacht-rental' ) .''. trim($post_meta['player_club']) .'</span>' : '')
										. (!empty($post_meta['player_age']) && !yacht_rental_param_is_inherit($post_meta['player_age']) && $post_meta['player_type'] == 'player' ? '<span class="player_age">'. esc_html__( 'Age: ', 'yacht-rental' ) .''. trim($post_meta['player_age']) .'</span>' : '')
									.'</div>';
							}
							...
							yacht_rental_show_layout($reviews_summary); ?>
							<div class="post_descr">
								<?php yacht_rental_show_layout($post_data['post_excerpt']); ?>
							</div>
							<?php if ($post_data['post_link'] != '') { ?>
								<div class="post_buttons">
									<?php if(function_exists('yacht_rental_sc_button')) yacht_rental_show_layout(yacht_rental_sc_button(array('size'=>'small', 'link'=>$post_data['post_link']), esc_html__('LEARN MORE', 'yacht-rental'))); ?>
								</div>
							<?php } ?>
						</div>
					</div>				<!-- /.post_content -->
			...
			if ($style=='alter') {
				...
				$alter_size = explode('_', yacht_rental_get_custom_option('alter_thumb_size', '1_1', $post_data['post_id'], $post_data['post_type']));	// !!!!!! Get option from specified post
				$alter_size[0] = max(1, $alter_size[0]);
				$alter_size[1] = max(1, empty($alter_size[1]) ? 1 : $alter_size[1]);
				$post_data['post_thumb'] = yacht_rental_get_resized_image_tag($post_data['post_attachment'], 
				...
				<<?php yacht_rental_show_layout($tag); ?> class="post_item post_item_<?php echo esc_attr($style); ?> post_item_<?php echo esc_attr($post_options['layout']); ?>
					...
					<div class="post_content isotope_item_content<?php
					 ...
					 ?>">
						...
						?>
						<div class="post_featured img <?php yacht_rental_show_layout($post_data['post_type'] == 'matches' ? 'matches_hover' : '');?>">
							<?php 
								...
								<div class="post_descr">
									...
									if ($post_data['post_type'] == 'players') {
										...
										echo '<div class="match_date">'
												...
												.(!empty($post_meta['match_time']) && !yacht_rental_param_is_inherit($post_meta['match_time']) ?  ' ' . trim($post_meta['match_time']) : '')
											.'</div>';
										?><div class="post_category"><?php yacht_rental_show_layout($category); ?></div><?php
									} else { 
										?><div class="post_category"><?php yacht_rental_show_layout($category); ?></div><?php
									}
									yacht_rental_show_layout($reviews_summary); ?>
						...
						if ($style!='colored') {
							...
							<div class="post_info_wrap info"><div class="info-back">
								...
								<?php
								if ($show_title) {
									?><h4 class="post_title"><?php yacht_rental_show_layout($post_data['post_title'], $link_start, $link_end); ?></h4><?php
								}
								?>
								...
								<?php
									...
									if ($post_data['post_protected']) {
										yacht_rental_show_layout($post_data['post_excerpt'], $link_start, $link_end);
									} else {
										...
										if ($post_data['post_excerpt']) {
											echo in_array($post_data['post_format'], array('quote', 'link', 'chat', 'aside', 'status')) 
												...
												: '<p>' . ($link_start) 
													. (yacht_rental_strpos($post_options['hover'], 'square')!==false
														? strip_tags($post_data['post_excerpt'])
														: trim(yacht_rental_strshort($post_data['post_excerpt'], isset($post_options['descr']) ? $post_options['descr'] : yacht_rental_get_custom_option('post_excerpt_maxlength_masonry'))) 
														)
													. ($link_end) . '</p>';
										}
